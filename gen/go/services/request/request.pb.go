// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: services/request/request.proto

package requestv1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type RequestType int32

const (
	RequestType_REQUEST_TYPE_UNSPECIFIED RequestType = 0
	RequestType_REQUEST_TYPE_REPAIR      RequestType = 1 // Ремонт
	RequestType_REQUEST_TYPE_PARTS       RequestType = 2 // Запчасти
)

// Enum value maps for RequestType.
var (
	RequestType_name = map[int32]string{
		0: "REQUEST_TYPE_UNSPECIFIED",
		1: "REQUEST_TYPE_REPAIR",
		2: "REQUEST_TYPE_PARTS",
	}
	RequestType_value = map[string]int32{
		"REQUEST_TYPE_UNSPECIFIED": 0,
		"REQUEST_TYPE_REPAIR":      1,
		"REQUEST_TYPE_PARTS":       2,
	}
)

func (x RequestType) Enum() *RequestType {
	p := new(RequestType)
	*p = x
	return p
}

func (x RequestType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RequestType) Descriptor() protoreflect.EnumDescriptor {
	return file_services_request_request_proto_enumTypes[0].Descriptor()
}

func (RequestType) Type() protoreflect.EnumType {
	return &file_services_request_request_proto_enumTypes[0]
}

func (x RequestType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RequestType.Descriptor instead.
func (RequestType) EnumDescriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{0}
}

type RequestStatus int32

const (
	RequestStatus_REQUEST_STATUS_UNSPECIFIED RequestStatus = 0
	RequestStatus_REQUEST_STATUS_MODERATION  RequestStatus = 1
	RequestStatus_REQUEST_STATUS_PUBLISHED   RequestStatus = 2
	RequestStatus_REQUEST_STATUS_DELETED     RequestStatus = 3
	// Closed by the customer after picking an organization and (for repair)
	// leaving a review, or after parts/workshop receipt confirmation.
	RequestStatus_REQUEST_STATUS_CLOSED RequestStatus = 4
	// Temporarily hidden by the customer ("на паузе"). Stays classified and can
	// be republished at any time (→ PUBLISHED). Invisible to the org/СТО feed.
	RequestStatus_REQUEST_STATUS_PAUSED RequestStatus = 5
)

// Enum value maps for RequestStatus.
var (
	RequestStatus_name = map[int32]string{
		0: "REQUEST_STATUS_UNSPECIFIED",
		1: "REQUEST_STATUS_MODERATION",
		2: "REQUEST_STATUS_PUBLISHED",
		3: "REQUEST_STATUS_DELETED",
		4: "REQUEST_STATUS_CLOSED",
		5: "REQUEST_STATUS_PAUSED",
	}
	RequestStatus_value = map[string]int32{
		"REQUEST_STATUS_UNSPECIFIED": 0,
		"REQUEST_STATUS_MODERATION":  1,
		"REQUEST_STATUS_PUBLISHED":   2,
		"REQUEST_STATUS_DELETED":     3,
		"REQUEST_STATUS_CLOSED":      4,
		"REQUEST_STATUS_PAUSED":      5,
	}
)

func (x RequestStatus) Enum() *RequestStatus {
	p := new(RequestStatus)
	*p = x
	return p
}

func (x RequestStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RequestStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_services_request_request_proto_enumTypes[1].Descriptor()
}

func (RequestStatus) Type() protoreflect.EnumType {
	return &file_services_request_request_proto_enumTypes[1]
}

func (x RequestStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RequestStatus.Descriptor instead.
func (RequestStatus) EnumDescriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{1}
}

type RequestEscrowState int32

const (
	RequestEscrowState_REQUEST_ESCROW_STATE_UNSPECIFIED   RequestEscrowState = 0
	RequestEscrowState_REQUEST_ESCROW_STATE_INITIATED     RequestEscrowState = 1
	RequestEscrowState_REQUEST_ESCROW_STATE_HELD          RequestEscrowState = 2
	RequestEscrowState_REQUEST_ESCROW_STATE_WORK_DECLARED RequestEscrowState = 3
	RequestEscrowState_REQUEST_ESCROW_STATE_DISPUTED      RequestEscrowState = 4
	RequestEscrowState_REQUEST_ESCROW_STATE_RELEASED      RequestEscrowState = 5
	RequestEscrowState_REQUEST_ESCROW_STATE_RETURNED      RequestEscrowState = 6
)

// Enum value maps for RequestEscrowState.
var (
	RequestEscrowState_name = map[int32]string{
		0: "REQUEST_ESCROW_STATE_UNSPECIFIED",
		1: "REQUEST_ESCROW_STATE_INITIATED",
		2: "REQUEST_ESCROW_STATE_HELD",
		3: "REQUEST_ESCROW_STATE_WORK_DECLARED",
		4: "REQUEST_ESCROW_STATE_DISPUTED",
		5: "REQUEST_ESCROW_STATE_RELEASED",
		6: "REQUEST_ESCROW_STATE_RETURNED",
	}
	RequestEscrowState_value = map[string]int32{
		"REQUEST_ESCROW_STATE_UNSPECIFIED":   0,
		"REQUEST_ESCROW_STATE_INITIATED":     1,
		"REQUEST_ESCROW_STATE_HELD":          2,
		"REQUEST_ESCROW_STATE_WORK_DECLARED": 3,
		"REQUEST_ESCROW_STATE_DISPUTED":      4,
		"REQUEST_ESCROW_STATE_RELEASED":      5,
		"REQUEST_ESCROW_STATE_RETURNED":      6,
	}
)

func (x RequestEscrowState) Enum() *RequestEscrowState {
	p := new(RequestEscrowState)
	*p = x
	return p
}

func (x RequestEscrowState) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RequestEscrowState) Descriptor() protoreflect.EnumDescriptor {
	return file_services_request_request_proto_enumTypes[2].Descriptor()
}

func (RequestEscrowState) Type() protoreflect.EnumType {
	return &file_services_request_request_proto_enumTypes[2]
}

func (x RequestEscrowState) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RequestEscrowState.Descriptor instead.
func (RequestEscrowState) EnumDescriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{2}
}

// RequestKind distinguishes a regular marketplace request from an insurance
// request (created by an admin on behalf of an insurer). Default UNSPECIFIED is
// treated as REGULAR by consumers.
type RequestKind int32

const (
	RequestKind_REQUEST_KIND_UNSPECIFIED RequestKind = 0
	RequestKind_REQUEST_KIND_REGULAR     RequestKind = 1
	RequestKind_REQUEST_KIND_INSURANCE   RequestKind = 2
)

// Enum value maps for RequestKind.
var (
	RequestKind_name = map[int32]string{
		0: "REQUEST_KIND_UNSPECIFIED",
		1: "REQUEST_KIND_REGULAR",
		2: "REQUEST_KIND_INSURANCE",
	}
	RequestKind_value = map[string]int32{
		"REQUEST_KIND_UNSPECIFIED": 0,
		"REQUEST_KIND_REGULAR":     1,
		"REQUEST_KIND_INSURANCE":   2,
	}
)

func (x RequestKind) Enum() *RequestKind {
	p := new(RequestKind)
	*p = x
	return p
}

func (x RequestKind) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RequestKind) Descriptor() protoreflect.EnumDescriptor {
	return file_services_request_request_proto_enumTypes[3].Descriptor()
}

func (RequestKind) Type() protoreflect.EnumType {
	return &file_services_request_request_proto_enumTypes[3]
}

func (x RequestKind) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RequestKind.Descriptor instead.
func (RequestKind) EnumDescriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{3}
}

// AssignmentMode controls how an insurance request is assigned to a СТО:
// MANUAL — admin picks the org; AUTO_FIRST — the first targeted org to claim wins.
type AssignmentMode int32

const (
	AssignmentMode_ASSIGNMENT_MODE_UNSPECIFIED AssignmentMode = 0
	AssignmentMode_ASSIGNMENT_MODE_MANUAL      AssignmentMode = 1
	AssignmentMode_ASSIGNMENT_MODE_AUTO_FIRST  AssignmentMode = 2
)

// Enum value maps for AssignmentMode.
var (
	AssignmentMode_name = map[int32]string{
		0: "ASSIGNMENT_MODE_UNSPECIFIED",
		1: "ASSIGNMENT_MODE_MANUAL",
		2: "ASSIGNMENT_MODE_AUTO_FIRST",
	}
	AssignmentMode_value = map[string]int32{
		"ASSIGNMENT_MODE_UNSPECIFIED": 0,
		"ASSIGNMENT_MODE_MANUAL":      1,
		"ASSIGNMENT_MODE_AUTO_FIRST":  2,
	}
)

func (x AssignmentMode) Enum() *AssignmentMode {
	p := new(AssignmentMode)
	*p = x
	return p
}

func (x AssignmentMode) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (AssignmentMode) Descriptor() protoreflect.EnumDescriptor {
	return file_services_request_request_proto_enumTypes[4].Descriptor()
}

func (AssignmentMode) Type() protoreflect.EnumType {
	return &file_services_request_request_proto_enumTypes[4]
}

func (x AssignmentMode) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use AssignmentMode.Descriptor instead.
func (AssignmentMode) EnumDescriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{4}
}

// EstimateItem is one line of the pre-agreed insurance repair estimate (смета).
type EstimateItem struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`     // работа
	Hours         float64                `protobuf:"fixed64,2,opt,name=hours,proto3" json:"hours,omitempty"` // нормо-часы
	Price         int64                  `protobuf:"varint,3,opt,name=price,proto3" json:"price,omitempty"`  // тиыны, за позицию
	Qty           int32                  `protobuf:"varint,4,opt,name=qty,proto3" json:"qty,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EstimateItem) Reset() {
	*x = EstimateItem{}
	mi := &file_services_request_request_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EstimateItem) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EstimateItem) ProtoMessage() {}

func (x *EstimateItem) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EstimateItem.ProtoReflect.Descriptor instead.
func (*EstimateItem) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{0}
}

func (x *EstimateItem) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *EstimateItem) GetHours() float64 {
	if x != nil {
		return x.Hours
	}
	return 0
}

func (x *EstimateItem) GetPrice() int64 {
	if x != nil {
		return x.Price
	}
	return 0
}

func (x *EstimateItem) GetQty() int32 {
	if x != nil {
		return x.Qty
	}
	return 0
}

type Request struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Id     string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"` // UUID string
	Type   RequestType            `protobuf:"varint,2,opt,name=type,proto3,enum=services.request.v1.RequestType" json:"type,omitempty"`
	Status RequestStatus          `protobuf:"varint,3,opt,name=status,proto3,enum=services.request.v1.RequestStatus" json:"status,omitempty"`
	UserId int64                  `protobuf:"varint,4,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	// NSI (Группа → Категории)
	GroupId     int64   `protobuf:"varint,5,opt,name=group_id,json=groupId,proto3" json:"group_id,omitempty"`                    // СТО, Кузовные, Запчасти
	CategoryIds []int64 `protobuf:"varint,6,rep,packed,name=category_ids,json=categoryIds,proto3" json:"category_ids,omitempty"` // Может быть несколько
	// Car details (для матчинга)
	CarMakeId       int64  `protobuf:"varint,7,opt,name=car_make_id,json=carMakeId,proto3" json:"car_make_id,omitempty"`
	CarModelId      int64  `protobuf:"varint,8,opt,name=car_model_id,json=carModelId,proto3" json:"car_model_id,omitempty"`
	CarGenerationId *int64 `protobuf:"varint,9,opt,name=car_generation_id,json=carGenerationId,proto3,oneof" json:"car_generation_id,omitempty"`
	Year            int32  `protobuf:"varint,10,opt,name=year,proto3" json:"year,omitempty"`
	// Description
	Note   string   `protobuf:"bytes,11,opt,name=note,proto3" json:"note,omitempty"`
	Photos []string `protobuf:"bytes,12,rep,name=photos,proto3" json:"photos,omitempty"`
	// Location
	CityId      int64                  `protobuf:"varint,13,opt,name=city_id,json=cityId,proto3" json:"city_id,omitempty"`
	Address     string                 `protobuf:"bytes,14,opt,name=address,proto3" json:"address,omitempty"`
	Latitude    float64                `protobuf:"fixed64,15,opt,name=latitude,proto3" json:"latitude,omitempty"`
	Longitude   float64                `protobuf:"fixed64,16,opt,name=longitude,proto3" json:"longitude,omitempty"`
	CreatedAt   *timestamppb.Timestamp `protobuf:"bytes,17,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	UpdatedAt   *timestamppb.Timestamp `protobuf:"bytes,18,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	PublishedAt *timestamppb.Timestamp `protobuf:"bytes,19,opt,name=published_at,json=publishedAt,proto3,oneof" json:"published_at,omitempty"`
	// Workshop integration (parts request from repair order)
	RepairOrderId *int64  `protobuf:"varint,20,opt,name=repair_order_id,json=repairOrderId,proto3,oneof" json:"repair_order_id,omitempty"` // Linked repair order in cg-workshop
	OrgId         *string `protobuf:"bytes,21,opt,name=org_id,json=orgId,proto3,oneof" json:"org_id,omitempty"`                            // Organization UUID creating the request
	// Enriched field: set when listing with organization_id context
	IsNew bool `protobuf:"varint,22,opt,name=is_new,json=isNew,proto3" json:"is_new,omitempty"` // true if this request has not been viewed by the requesting organization
	// Enriched field: number of bids (откликов) on this request. Populated by the
	// list/get queries via a LATERAL count subquery (no row multiplication).
	BidsCount int32 `protobuf:"varint,23,opt,name=bids_count,json=bidsCount,proto3" json:"bids_count,omitempty"`
	// Pause metadata — set only when status == REQUEST_STATUS_PAUSED.
	PausedAt *timestamppb.Timestamp `protobuf:"bytes,24,opt,name=paused_at,json=pausedAt,proto3,oneof" json:"paused_at,omitempty"`
	// UUID of org the customer paused "for" (mirrors legacy paused_by_service_id);
	// empty when self-paused.
	PausedByOrgId *string `protobuf:"bytes,25,opt,name=paused_by_org_id,json=pausedByOrgId,proto3,oneof" json:"paused_by_org_id,omitempty"`
	// Legacy ad.id (numeric) this request was migrated from — bridges still-legacy
	// cg_web flows (reviews, chat responses, notifications) that key off the old id.
	// 0/absent for natively-created requests.
	LegacyId *int64 `protobuf:"varint,26,opt,name=legacy_id,json=legacyId,proto3,oneof" json:"legacy_id,omitempty"`
	// Garage car (cg-users garage) this request was created for. Persisted so
	// request history can be queried per specific car, beyond the make/model/year
	// snapshot. Absent for requests created without a garage car.
	GarageCarId *int64 `protobuf:"varint,27,opt,name=garage_car_id,json=garageCarId,proto3,oneof" json:"garage_car_id,omitempty"`
	// Insurance-request fields — meaningful only when kind == REQUEST_KIND_INSURANCE.
	// Regular requests leave kind UNSPECIFIED (consumers treat that as REGULAR).
	// NB: insurance_price (X) and client contacts are admin-only and intentionally
	// NOT carried here — fetch them via GetRequestInsuranceInfo (RequestInsuranceInfo).
	Kind RequestKind `protobuf:"varint,28,opt,name=kind,proto3,enum=services.request.v1.RequestKind" json:"kind,omitempty"`
	// Публичная выплата СТО (Y), тиыны — safe to show in the СТО feed.
	PayoutPrice    *int64                 `protobuf:"varint,29,opt,name=payout_price,json=payoutPrice,proto3,oneof" json:"payout_price,omitempty"`
	Deadline       *timestamppb.Timestamp `protobuf:"bytes,30,opt,name=deadline,proto3,oneof" json:"deadline,omitempty"`
	AssignmentMode AssignmentMode         `protobuf:"varint,31,opt,name=assignment_mode,json=assignmentMode,proto3,enum=services.request.v1.AssignmentMode" json:"assignment_mode,omitempty"`
	WorkOrderId    *int64                 `protobuf:"varint,32,opt,name=work_order_id,json=workOrderId,proto3,oneof" json:"work_order_id,omitempty"` // RepairOrder.id из cg-workshop
	EstimateItems  []*EstimateItem        `protobuf:"bytes,33,rep,name=estimate_items,json=estimateItems,proto3" json:"estimate_items,omitempty"`
	// Winning org once the insurance request is claimed (auto_first CAS or
	// manual accept). Not sensitive — the claim outcome is already visible to
	// partners; admin UIs use it to render "В работе" while request_status
	// itself stays 'published' (there is no matched-like status in the enum).
	ClaimedByOrgId *string `protobuf:"bytes,34,opt,name=claimed_by_org_id,json=claimedByOrgId,proto3,oneof" json:"claimed_by_org_id,omitempty"`
	// Safe-payment projection. ever_held is monotonic and remains true after a
	// refund, dispute or release so ordinary close and stale close cannot bypass
	// a deal that once held money.
	EverHeld               bool               `protobuf:"varint,35,opt,name=ever_held,json=everHeld,proto3" json:"ever_held,omitempty"`
	SelectedBidId          *int64             `protobuf:"varint,36,opt,name=selected_bid_id,json=selectedBidId,proto3,oneof" json:"selected_bid_id,omitempty"`
	SelectedOrganizationId *string            `protobuf:"bytes,37,opt,name=selected_organization_id,json=selectedOrganizationId,proto3,oneof" json:"selected_organization_id,omitempty"`
	EscrowState            RequestEscrowState `protobuf:"varint,38,opt,name=escrow_state,json=escrowState,proto3,enum=services.request.v1.RequestEscrowState" json:"escrow_state,omitempty"`
	EscrowTransactionId    *int64             `protobuf:"varint,39,opt,name=escrow_transaction_id,json=escrowTransactionId,proto3,oneof" json:"escrow_transaction_id,omitempty"`
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *Request) Reset() {
	*x = Request{}
	mi := &file_services_request_request_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Request) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Request) ProtoMessage() {}

func (x *Request) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Request.ProtoReflect.Descriptor instead.
func (*Request) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{1}
}

func (x *Request) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Request) GetType() RequestType {
	if x != nil {
		return x.Type
	}
	return RequestType_REQUEST_TYPE_UNSPECIFIED
}

func (x *Request) GetStatus() RequestStatus {
	if x != nil {
		return x.Status
	}
	return RequestStatus_REQUEST_STATUS_UNSPECIFIED
}

func (x *Request) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *Request) GetGroupId() int64 {
	if x != nil {
		return x.GroupId
	}
	return 0
}

func (x *Request) GetCategoryIds() []int64 {
	if x != nil {
		return x.CategoryIds
	}
	return nil
}

func (x *Request) GetCarMakeId() int64 {
	if x != nil {
		return x.CarMakeId
	}
	return 0
}

func (x *Request) GetCarModelId() int64 {
	if x != nil {
		return x.CarModelId
	}
	return 0
}

func (x *Request) GetCarGenerationId() int64 {
	if x != nil && x.CarGenerationId != nil {
		return *x.CarGenerationId
	}
	return 0
}

func (x *Request) GetYear() int32 {
	if x != nil {
		return x.Year
	}
	return 0
}

func (x *Request) GetNote() string {
	if x != nil {
		return x.Note
	}
	return ""
}

func (x *Request) GetPhotos() []string {
	if x != nil {
		return x.Photos
	}
	return nil
}

func (x *Request) GetCityId() int64 {
	if x != nil {
		return x.CityId
	}
	return 0
}

func (x *Request) GetAddress() string {
	if x != nil {
		return x.Address
	}
	return ""
}

func (x *Request) GetLatitude() float64 {
	if x != nil {
		return x.Latitude
	}
	return 0
}

func (x *Request) GetLongitude() float64 {
	if x != nil {
		return x.Longitude
	}
	return 0
}

func (x *Request) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *Request) GetUpdatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

func (x *Request) GetPublishedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.PublishedAt
	}
	return nil
}

func (x *Request) GetRepairOrderId() int64 {
	if x != nil && x.RepairOrderId != nil {
		return *x.RepairOrderId
	}
	return 0
}

func (x *Request) GetOrgId() string {
	if x != nil && x.OrgId != nil {
		return *x.OrgId
	}
	return ""
}

func (x *Request) GetIsNew() bool {
	if x != nil {
		return x.IsNew
	}
	return false
}

func (x *Request) GetBidsCount() int32 {
	if x != nil {
		return x.BidsCount
	}
	return 0
}

func (x *Request) GetPausedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.PausedAt
	}
	return nil
}

func (x *Request) GetPausedByOrgId() string {
	if x != nil && x.PausedByOrgId != nil {
		return *x.PausedByOrgId
	}
	return ""
}

func (x *Request) GetLegacyId() int64 {
	if x != nil && x.LegacyId != nil {
		return *x.LegacyId
	}
	return 0
}

func (x *Request) GetGarageCarId() int64 {
	if x != nil && x.GarageCarId != nil {
		return *x.GarageCarId
	}
	return 0
}

func (x *Request) GetKind() RequestKind {
	if x != nil {
		return x.Kind
	}
	return RequestKind_REQUEST_KIND_UNSPECIFIED
}

func (x *Request) GetPayoutPrice() int64 {
	if x != nil && x.PayoutPrice != nil {
		return *x.PayoutPrice
	}
	return 0
}

func (x *Request) GetDeadline() *timestamppb.Timestamp {
	if x != nil {
		return x.Deadline
	}
	return nil
}

func (x *Request) GetAssignmentMode() AssignmentMode {
	if x != nil {
		return x.AssignmentMode
	}
	return AssignmentMode_ASSIGNMENT_MODE_UNSPECIFIED
}

func (x *Request) GetWorkOrderId() int64 {
	if x != nil && x.WorkOrderId != nil {
		return *x.WorkOrderId
	}
	return 0
}

func (x *Request) GetEstimateItems() []*EstimateItem {
	if x != nil {
		return x.EstimateItems
	}
	return nil
}

func (x *Request) GetClaimedByOrgId() string {
	if x != nil && x.ClaimedByOrgId != nil {
		return *x.ClaimedByOrgId
	}
	return ""
}

func (x *Request) GetEverHeld() bool {
	if x != nil {
		return x.EverHeld
	}
	return false
}

func (x *Request) GetSelectedBidId() int64 {
	if x != nil && x.SelectedBidId != nil {
		return *x.SelectedBidId
	}
	return 0
}

func (x *Request) GetSelectedOrganizationId() string {
	if x != nil && x.SelectedOrganizationId != nil {
		return *x.SelectedOrganizationId
	}
	return ""
}

func (x *Request) GetEscrowState() RequestEscrowState {
	if x != nil {
		return x.EscrowState
	}
	return RequestEscrowState_REQUEST_ESCROW_STATE_UNSPECIFIED
}

func (x *Request) GetEscrowTransactionId() int64 {
	if x != nil && x.EscrowTransactionId != nil {
		return *x.EscrowTransactionId
	}
	return 0
}

// CreateRequest
type CreateRequestRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Type            RequestType            `protobuf:"varint,1,opt,name=type,proto3,enum=services.request.v1.RequestType" json:"type,omitempty"`
	UserId          int64                  `protobuf:"varint,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	GroupId         int64                  `protobuf:"varint,3,opt,name=group_id,json=groupId,proto3" json:"group_id,omitempty"`
	CategoryIds     []int64                `protobuf:"varint,4,rep,packed,name=category_ids,json=categoryIds,proto3" json:"category_ids,omitempty"`
	CarMakeId       int64                  `protobuf:"varint,5,opt,name=car_make_id,json=carMakeId,proto3" json:"car_make_id,omitempty"`
	CarModelId      int64                  `protobuf:"varint,6,opt,name=car_model_id,json=carModelId,proto3" json:"car_model_id,omitempty"`
	CarGenerationId *int64                 `protobuf:"varint,7,opt,name=car_generation_id,json=carGenerationId,proto3,oneof" json:"car_generation_id,omitempty"`
	Year            int32                  `protobuf:"varint,8,opt,name=year,proto3" json:"year,omitempty"`
	Note            string                 `protobuf:"bytes,9,opt,name=note,proto3" json:"note,omitempty"`
	Photos          []string               `protobuf:"bytes,10,rep,name=photos,proto3" json:"photos,omitempty"`
	CityId          int64                  `protobuf:"varint,11,opt,name=city_id,json=cityId,proto3" json:"city_id,omitempty"`
	Address         string                 `protobuf:"bytes,12,opt,name=address,proto3" json:"address,omitempty"`
	Latitude        float64                `protobuf:"fixed64,13,opt,name=latitude,proto3" json:"latitude,omitempty"`
	Longitude       float64                `protobuf:"fixed64,14,opt,name=longitude,proto3" json:"longitude,omitempty"`
	Publish         bool                   `protobuf:"varint,15,opt,name=publish,proto3" json:"publish,omitempty"` // Если true, статус = "moderation"
	// Workshop integration (parts request from repair order)
	RepairOrderId *int64  `protobuf:"varint,16,opt,name=repair_order_id,json=repairOrderId,proto3,oneof" json:"repair_order_id,omitempty"` // Linked repair order in cg-workshop
	OrgId         *string `protobuf:"bytes,17,opt,name=org_id,json=orgId,proto3,oneof" json:"org_id,omitempty"`                            // Organization UUID creating the request
	// Garage car (cg-users garage) the request is created for. The BFF also uses
	// it to fill car_make_id/model/year, but it is persisted so request history
	// can be queried per specific car.
	GarageCarId *int64 `protobuf:"varint,18,opt,name=garage_car_id,json=garageCarId,proto3,oneof" json:"garage_car_id,omitempty"`
	// Insurance-request creation — set only when kind == REQUEST_KIND_INSURANCE.
	Kind           RequestKind            `protobuf:"varint,19,opt,name=kind,proto3,enum=services.request.v1.RequestKind" json:"kind,omitempty"`
	PayoutPrice    *int64                 `protobuf:"varint,20,opt,name=payout_price,json=payoutPrice,proto3,oneof" json:"payout_price,omitempty"` // Y, тиыны — публичная выплата СТО
	Deadline       *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=deadline,proto3,oneof" json:"deadline,omitempty"`
	AssignmentMode AssignmentMode         `protobuf:"varint,22,opt,name=assignment_mode,json=assignmentMode,proto3,enum=services.request.v1.AssignmentMode" json:"assignment_mode,omitempty"`
	WorkOrderId    *int64                 `protobuf:"varint,23,opt,name=work_order_id,json=workOrderId,proto3,oneof" json:"work_order_id,omitempty"` // RepairOrder.id из cg-workshop
	EstimateItems  []*EstimateItem        `protobuf:"bytes,24,rep,name=estimate_items,json=estimateItems,proto3" json:"estimate_items,omitempty"`
	TargetOrgIds   []string               `protobuf:"bytes,25,rep,name=target_org_ids,json=targetOrgIds,proto3" json:"target_org_ids,omitempty"`        // snapshot аудитории (org UUIDs)
	InsuranceInfo  *InsuranceInfoInput    `protobuf:"bytes,26,opt,name=insurance_info,json=insuranceInfo,proto3,oneof" json:"insurance_info,omitempty"` // админ-only: X + контакты клиента
	// Caller-generated token for safely retrying an ambiguous create. The owner
	// service scopes it to the authenticated principal and rejects reuse with a
	// different payload. Maximum 255 bytes; empty keeps legacy behavior.
	IdempotencyKey string `protobuf:"bytes,27,opt,name=idempotency_key,json=idempotencyKey,proto3" json:"idempotency_key,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *CreateRequestRequest) Reset() {
	*x = CreateRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateRequestRequest) ProtoMessage() {}

func (x *CreateRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateRequestRequest.ProtoReflect.Descriptor instead.
func (*CreateRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{2}
}

func (x *CreateRequestRequest) GetType() RequestType {
	if x != nil {
		return x.Type
	}
	return RequestType_REQUEST_TYPE_UNSPECIFIED
}

func (x *CreateRequestRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *CreateRequestRequest) GetGroupId() int64 {
	if x != nil {
		return x.GroupId
	}
	return 0
}

func (x *CreateRequestRequest) GetCategoryIds() []int64 {
	if x != nil {
		return x.CategoryIds
	}
	return nil
}

func (x *CreateRequestRequest) GetCarMakeId() int64 {
	if x != nil {
		return x.CarMakeId
	}
	return 0
}

func (x *CreateRequestRequest) GetCarModelId() int64 {
	if x != nil {
		return x.CarModelId
	}
	return 0
}

func (x *CreateRequestRequest) GetCarGenerationId() int64 {
	if x != nil && x.CarGenerationId != nil {
		return *x.CarGenerationId
	}
	return 0
}

func (x *CreateRequestRequest) GetYear() int32 {
	if x != nil {
		return x.Year
	}
	return 0
}

func (x *CreateRequestRequest) GetNote() string {
	if x != nil {
		return x.Note
	}
	return ""
}

func (x *CreateRequestRequest) GetPhotos() []string {
	if x != nil {
		return x.Photos
	}
	return nil
}

func (x *CreateRequestRequest) GetCityId() int64 {
	if x != nil {
		return x.CityId
	}
	return 0
}

func (x *CreateRequestRequest) GetAddress() string {
	if x != nil {
		return x.Address
	}
	return ""
}

func (x *CreateRequestRequest) GetLatitude() float64 {
	if x != nil {
		return x.Latitude
	}
	return 0
}

func (x *CreateRequestRequest) GetLongitude() float64 {
	if x != nil {
		return x.Longitude
	}
	return 0
}

func (x *CreateRequestRequest) GetPublish() bool {
	if x != nil {
		return x.Publish
	}
	return false
}

func (x *CreateRequestRequest) GetRepairOrderId() int64 {
	if x != nil && x.RepairOrderId != nil {
		return *x.RepairOrderId
	}
	return 0
}

func (x *CreateRequestRequest) GetOrgId() string {
	if x != nil && x.OrgId != nil {
		return *x.OrgId
	}
	return ""
}

func (x *CreateRequestRequest) GetGarageCarId() int64 {
	if x != nil && x.GarageCarId != nil {
		return *x.GarageCarId
	}
	return 0
}

func (x *CreateRequestRequest) GetKind() RequestKind {
	if x != nil {
		return x.Kind
	}
	return RequestKind_REQUEST_KIND_UNSPECIFIED
}

func (x *CreateRequestRequest) GetPayoutPrice() int64 {
	if x != nil && x.PayoutPrice != nil {
		return *x.PayoutPrice
	}
	return 0
}

func (x *CreateRequestRequest) GetDeadline() *timestamppb.Timestamp {
	if x != nil {
		return x.Deadline
	}
	return nil
}

func (x *CreateRequestRequest) GetAssignmentMode() AssignmentMode {
	if x != nil {
		return x.AssignmentMode
	}
	return AssignmentMode_ASSIGNMENT_MODE_UNSPECIFIED
}

func (x *CreateRequestRequest) GetWorkOrderId() int64 {
	if x != nil && x.WorkOrderId != nil {
		return *x.WorkOrderId
	}
	return 0
}

func (x *CreateRequestRequest) GetEstimateItems() []*EstimateItem {
	if x != nil {
		return x.EstimateItems
	}
	return nil
}

func (x *CreateRequestRequest) GetTargetOrgIds() []string {
	if x != nil {
		return x.TargetOrgIds
	}
	return nil
}

func (x *CreateRequestRequest) GetInsuranceInfo() *InsuranceInfoInput {
	if x != nil {
		return x.InsuranceInfo
	}
	return nil
}

func (x *CreateRequestRequest) GetIdempotencyKey() string {
	if x != nil {
		return x.IdempotencyKey
	}
	return ""
}

type CreateRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateRequestResponse) Reset() {
	*x = CreateRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateRequestResponse) ProtoMessage() {}

func (x *CreateRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateRequestResponse.ProtoReflect.Descriptor instead.
func (*CreateRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{3}
}

func (x *CreateRequestResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

// GetRequest
type GetRequestRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID string
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRequestRequest) Reset() {
	*x = GetRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRequestRequest) ProtoMessage() {}

func (x *GetRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRequestRequest.ProtoReflect.Descriptor instead.
func (*GetRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{4}
}

func (x *GetRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

type GetRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRequestResponse) Reset() {
	*x = GetRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRequestResponse) ProtoMessage() {}

func (x *GetRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRequestResponse.ProtoReflect.Descriptor instead.
func (*GetRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{5}
}

func (x *GetRequestResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

// UpdateRequest
type UpdateRequestRequest struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	RequestId string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID string
	UserId    int64                  `protobuf:"varint,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	Note      *string                `protobuf:"bytes,3,opt,name=note,proto3,oneof" json:"note,omitempty"`
	Photos    []string               `protobuf:"bytes,4,rep,name=photos,proto3" json:"photos,omitempty"`
	Address   *string                `protobuf:"bytes,5,opt,name=address,proto3,oneof" json:"address,omitempty"`
	Latitude  *float64               `protobuf:"fixed64,6,opt,name=latitude,proto3,oneof" json:"latitude,omitempty"`
	Longitude *float64               `protobuf:"fixed64,7,opt,name=longitude,proto3,oneof" json:"longitude,omitempty"`
	// Car identity — editable so a manager can set the vehicle on a parts request
	// that was created without one (customer is contacted, car specified in admin).
	// Each is optional: absent = leave unchanged.
	CarMakeId       *int64 `protobuf:"varint,8,opt,name=car_make_id,json=carMakeId,proto3,oneof" json:"car_make_id,omitempty"`
	CarModelId      *int64 `protobuf:"varint,9,opt,name=car_model_id,json=carModelId,proto3,oneof" json:"car_model_id,omitempty"`
	CarGenerationId *int64 `protobuf:"varint,10,opt,name=car_generation_id,json=carGenerationId,proto3,oneof" json:"car_generation_id,omitempty"`
	Year            *int32 `protobuf:"varint,11,opt,name=year,proto3,oneof" json:"year,omitempty"`
	GarageCarId     *int64 `protobuf:"varint,12,opt,name=garage_car_id,json=garageCarId,proto3,oneof" json:"garage_car_id,omitempty"`
	// NSI classification — editable so a manager can correct the group (СТО/
	// кузовные/запчасти) and categories of a request. group_id is optional
	// (absent = unchanged); category_ids replaces the set when has_category_ids
	// is true (allows clearing to empty).
	GroupId        *int64  `protobuf:"varint,13,opt,name=group_id,json=groupId,proto3,oneof" json:"group_id,omitempty"`
	CategoryIds    []int64 `protobuf:"varint,14,rep,packed,name=category_ids,json=categoryIds,proto3" json:"category_ids,omitempty"`
	HasCategoryIds bool    `protobuf:"varint,15,opt,name=has_category_ids,json=hasCategoryIds,proto3" json:"has_category_ids,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *UpdateRequestRequest) Reset() {
	*x = UpdateRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateRequestRequest) ProtoMessage() {}

func (x *UpdateRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateRequestRequest.ProtoReflect.Descriptor instead.
func (*UpdateRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{6}
}

func (x *UpdateRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *UpdateRequestRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *UpdateRequestRequest) GetNote() string {
	if x != nil && x.Note != nil {
		return *x.Note
	}
	return ""
}

func (x *UpdateRequestRequest) GetPhotos() []string {
	if x != nil {
		return x.Photos
	}
	return nil
}

func (x *UpdateRequestRequest) GetAddress() string {
	if x != nil && x.Address != nil {
		return *x.Address
	}
	return ""
}

func (x *UpdateRequestRequest) GetLatitude() float64 {
	if x != nil && x.Latitude != nil {
		return *x.Latitude
	}
	return 0
}

func (x *UpdateRequestRequest) GetLongitude() float64 {
	if x != nil && x.Longitude != nil {
		return *x.Longitude
	}
	return 0
}

func (x *UpdateRequestRequest) GetCarMakeId() int64 {
	if x != nil && x.CarMakeId != nil {
		return *x.CarMakeId
	}
	return 0
}

func (x *UpdateRequestRequest) GetCarModelId() int64 {
	if x != nil && x.CarModelId != nil {
		return *x.CarModelId
	}
	return 0
}

func (x *UpdateRequestRequest) GetCarGenerationId() int64 {
	if x != nil && x.CarGenerationId != nil {
		return *x.CarGenerationId
	}
	return 0
}

func (x *UpdateRequestRequest) GetYear() int32 {
	if x != nil && x.Year != nil {
		return *x.Year
	}
	return 0
}

func (x *UpdateRequestRequest) GetGarageCarId() int64 {
	if x != nil && x.GarageCarId != nil {
		return *x.GarageCarId
	}
	return 0
}

func (x *UpdateRequestRequest) GetGroupId() int64 {
	if x != nil && x.GroupId != nil {
		return *x.GroupId
	}
	return 0
}

func (x *UpdateRequestRequest) GetCategoryIds() []int64 {
	if x != nil {
		return x.CategoryIds
	}
	return nil
}

func (x *UpdateRequestRequest) GetHasCategoryIds() bool {
	if x != nil {
		return x.HasCategoryIds
	}
	return false
}

type UpdateRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateRequestResponse) Reset() {
	*x = UpdateRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateRequestResponse) ProtoMessage() {}

func (x *UpdateRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateRequestResponse.ProtoReflect.Descriptor instead.
func (*UpdateRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{7}
}

func (x *UpdateRequestResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

// DeleteRequest
type DeleteRequestRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID string
	UserId        int64                  `protobuf:"varint,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteRequestRequest) Reset() {
	*x = DeleteRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteRequestRequest) ProtoMessage() {}

func (x *DeleteRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteRequestRequest.ProtoReflect.Descriptor instead.
func (*DeleteRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{8}
}

func (x *DeleteRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *DeleteRequestRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

type DeleteRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteRequestResponse) Reset() {
	*x = DeleteRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteRequestResponse) ProtoMessage() {}

func (x *DeleteRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteRequestResponse.ProtoReflect.Descriptor instead.
func (*DeleteRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{9}
}

func (x *DeleteRequestResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

// ListRequests
type ListRequestsRequest struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	Type                RequestType            `protobuf:"varint,1,opt,name=type,proto3,enum=services.request.v1.RequestType" json:"type,omitempty"`
	Status              RequestStatus          `protobuf:"varint,2,opt,name=status,proto3,enum=services.request.v1.RequestStatus" json:"status,omitempty"`
	UserId              int64                  `protobuf:"varint,3,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	GroupId             int64                  `protobuf:"varint,4,opt,name=group_id,json=groupId,proto3" json:"group_id,omitempty"`
	CategoryId          int64                  `protobuf:"varint,5,opt,name=category_id,json=categoryId,proto3" json:"category_id,omitempty"` // Фильтр по одной категории
	CityId              int64                  `protobuf:"varint,6,opt,name=city_id,json=cityId,proto3" json:"city_id,omitempty"`
	CarMakeId           int64                  `protobuf:"varint,7,opt,name=car_make_id,json=carMakeId,proto3" json:"car_make_id,omitempty"`
	Page                int32                  `protobuf:"varint,8,opt,name=page,proto3" json:"page,omitempty"`
	PageSize            int32                  `protobuf:"varint,9,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	SortBy              string                 `protobuf:"bytes,10,opt,name=sort_by,json=sortBy,proto3" json:"sort_by,omitempty"`                                                // created_at, published_at, unread_first
	RepairOrderId       *int64                 `protobuf:"varint,11,opt,name=repair_order_id,json=repairOrderId,proto3,oneof" json:"repair_order_id,omitempty"`                  // Filter by linked repair order
	OrganizationId      *string                `protobuf:"bytes,12,opt,name=organization_id,json=organizationId,proto3,oneof" json:"organization_id,omitempty"`                  // When set, enriches results with is_new flag; enables sort_by=unread_first
	OwnerOrganizationId *string                `protobuf:"bytes,13,opt,name=owner_organization_id,json=ownerOrganizationId,proto3,oneof" json:"owner_organization_id,omitempty"` // Filters requests whose authoritative org_id matches this organization
	Kind                *RequestKind           `protobuf:"varint,14,opt,name=kind,proto3,enum=services.request.v1.RequestKind,oneof" json:"kind,omitempty"`                      // Filter by request kind (admin list: e.g. only insurance requests)
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *ListRequestsRequest) Reset() {
	*x = ListRequestsRequest{}
	mi := &file_services_request_request_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRequestsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRequestsRequest) ProtoMessage() {}

func (x *ListRequestsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRequestsRequest.ProtoReflect.Descriptor instead.
func (*ListRequestsRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{10}
}

func (x *ListRequestsRequest) GetType() RequestType {
	if x != nil {
		return x.Type
	}
	return RequestType_REQUEST_TYPE_UNSPECIFIED
}

func (x *ListRequestsRequest) GetStatus() RequestStatus {
	if x != nil {
		return x.Status
	}
	return RequestStatus_REQUEST_STATUS_UNSPECIFIED
}

func (x *ListRequestsRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *ListRequestsRequest) GetGroupId() int64 {
	if x != nil {
		return x.GroupId
	}
	return 0
}

func (x *ListRequestsRequest) GetCategoryId() int64 {
	if x != nil {
		return x.CategoryId
	}
	return 0
}

func (x *ListRequestsRequest) GetCityId() int64 {
	if x != nil {
		return x.CityId
	}
	return 0
}

func (x *ListRequestsRequest) GetCarMakeId() int64 {
	if x != nil {
		return x.CarMakeId
	}
	return 0
}

func (x *ListRequestsRequest) GetPage() int32 {
	if x != nil {
		return x.Page
	}
	return 0
}

func (x *ListRequestsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListRequestsRequest) GetSortBy() string {
	if x != nil {
		return x.SortBy
	}
	return ""
}

func (x *ListRequestsRequest) GetRepairOrderId() int64 {
	if x != nil && x.RepairOrderId != nil {
		return *x.RepairOrderId
	}
	return 0
}

func (x *ListRequestsRequest) GetOrganizationId() string {
	if x != nil && x.OrganizationId != nil {
		return *x.OrganizationId
	}
	return ""
}

func (x *ListRequestsRequest) GetOwnerOrganizationId() string {
	if x != nil && x.OwnerOrganizationId != nil {
		return *x.OwnerOrganizationId
	}
	return ""
}

func (x *ListRequestsRequest) GetKind() RequestKind {
	if x != nil && x.Kind != nil {
		return *x.Kind
	}
	return RequestKind_REQUEST_KIND_UNSPECIFIED
}

type ListRequestsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Requests      []*Request             `protobuf:"bytes,1,rep,name=requests,proto3" json:"requests,omitempty"`
	Total         int32                  `protobuf:"varint,2,opt,name=total,proto3" json:"total,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRequestsResponse) Reset() {
	*x = ListRequestsResponse{}
	mi := &file_services_request_request_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRequestsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRequestsResponse) ProtoMessage() {}

func (x *ListRequestsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRequestsResponse.ProtoReflect.Descriptor instead.
func (*ListRequestsResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{11}
}

func (x *ListRequestsResponse) GetRequests() []*Request {
	if x != nil {
		return x.Requests
	}
	return nil
}

func (x *ListRequestsResponse) GetTotal() int32 {
	if x != nil {
		return x.Total
	}
	return 0
}

// ListPublishedPreviews
type ListPublishedPreviewsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	CategoryId    int64                  `protobuf:"varint,1,opt,name=category_id,json=categoryId,proto3" json:"category_id,omitempty"`
	CityId        int64                  `protobuf:"varint,2,opt,name=city_id,json=cityId,proto3" json:"city_id,omitempty"`
	CarMakeId     int64                  `protobuf:"varint,3,opt,name=car_make_id,json=carMakeId,proto3" json:"car_make_id,omitempty"`
	Page          int32                  `protobuf:"varint,4,opt,name=page,proto3" json:"page,omitempty"`
	PageSize      int32                  `protobuf:"varint,5,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedPreviewsRequest) Reset() {
	*x = ListPublishedPreviewsRequest{}
	mi := &file_services_request_request_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedPreviewsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedPreviewsRequest) ProtoMessage() {}

func (x *ListPublishedPreviewsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedPreviewsRequest.ProtoReflect.Descriptor instead.
func (*ListPublishedPreviewsRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{12}
}

func (x *ListPublishedPreviewsRequest) GetCategoryId() int64 {
	if x != nil {
		return x.CategoryId
	}
	return 0
}

func (x *ListPublishedPreviewsRequest) GetCityId() int64 {
	if x != nil {
		return x.CityId
	}
	return 0
}

func (x *ListPublishedPreviewsRequest) GetCarMakeId() int64 {
	if x != nil {
		return x.CarMakeId
	}
	return 0
}

func (x *ListPublishedPreviewsRequest) GetPage() int32 {
	if x != nil {
		return x.Page
	}
	return 0
}

func (x *ListPublishedPreviewsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

// PublishedRequestPreview intentionally contains only marketplace-card data.
type PublishedRequestPreview struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Type          RequestType            `protobuf:"varint,2,opt,name=type,proto3,enum=services.request.v1.RequestType" json:"type,omitempty"`
	GroupId       int64                  `protobuf:"varint,3,opt,name=group_id,json=groupId,proto3" json:"group_id,omitempty"`
	CategoryIds   []int64                `protobuf:"varint,4,rep,packed,name=category_ids,json=categoryIds,proto3" json:"category_ids,omitempty"`
	CarMakeId     int64                  `protobuf:"varint,5,opt,name=car_make_id,json=carMakeId,proto3" json:"car_make_id,omitempty"`
	CarModelId    int64                  `protobuf:"varint,6,opt,name=car_model_id,json=carModelId,proto3" json:"car_model_id,omitempty"`
	Year          int32                  `protobuf:"varint,7,opt,name=year,proto3" json:"year,omitempty"`
	Note          string                 `protobuf:"bytes,8,opt,name=note,proto3" json:"note,omitempty"`
	Photos        []string               `protobuf:"bytes,9,rep,name=photos,proto3" json:"photos,omitempty"`
	CityId        int64                  `protobuf:"varint,10,opt,name=city_id,json=cityId,proto3" json:"city_id,omitempty"`
	PublishedAt   *timestamppb.Timestamp `protobuf:"bytes,11,opt,name=published_at,json=publishedAt,proto3" json:"published_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PublishedRequestPreview) Reset() {
	*x = PublishedRequestPreview{}
	mi := &file_services_request_request_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublishedRequestPreview) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublishedRequestPreview) ProtoMessage() {}

func (x *PublishedRequestPreview) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublishedRequestPreview.ProtoReflect.Descriptor instead.
func (*PublishedRequestPreview) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{13}
}

func (x *PublishedRequestPreview) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *PublishedRequestPreview) GetType() RequestType {
	if x != nil {
		return x.Type
	}
	return RequestType_REQUEST_TYPE_UNSPECIFIED
}

func (x *PublishedRequestPreview) GetGroupId() int64 {
	if x != nil {
		return x.GroupId
	}
	return 0
}

func (x *PublishedRequestPreview) GetCategoryIds() []int64 {
	if x != nil {
		return x.CategoryIds
	}
	return nil
}

func (x *PublishedRequestPreview) GetCarMakeId() int64 {
	if x != nil {
		return x.CarMakeId
	}
	return 0
}

func (x *PublishedRequestPreview) GetCarModelId() int64 {
	if x != nil {
		return x.CarModelId
	}
	return 0
}

func (x *PublishedRequestPreview) GetYear() int32 {
	if x != nil {
		return x.Year
	}
	return 0
}

func (x *PublishedRequestPreview) GetNote() string {
	if x != nil {
		return x.Note
	}
	return ""
}

func (x *PublishedRequestPreview) GetPhotos() []string {
	if x != nil {
		return x.Photos
	}
	return nil
}

func (x *PublishedRequestPreview) GetCityId() int64 {
	if x != nil {
		return x.CityId
	}
	return 0
}

func (x *PublishedRequestPreview) GetPublishedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.PublishedAt
	}
	return nil
}

type ListPublishedPreviewsResponse struct {
	state         protoimpl.MessageState     `protogen:"open.v1"`
	Requests      []*PublishedRequestPreview `protobuf:"bytes,1,rep,name=requests,proto3" json:"requests,omitempty"`
	Total         int32                      `protobuf:"varint,2,opt,name=total,proto3" json:"total,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedPreviewsResponse) Reset() {
	*x = ListPublishedPreviewsResponse{}
	mi := &file_services_request_request_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedPreviewsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedPreviewsResponse) ProtoMessage() {}

func (x *ListPublishedPreviewsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedPreviewsResponse.ProtoReflect.Descriptor instead.
func (*ListPublishedPreviewsResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{14}
}

func (x *ListPublishedPreviewsResponse) GetRequests() []*PublishedRequestPreview {
	if x != nil {
		return x.Requests
	}
	return nil
}

func (x *ListPublishedPreviewsResponse) GetTotal() int32 {
	if x != nil {
		return x.Total
	}
	return 0
}

// SearchRequests
type SearchRequestsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         string                 `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	Type          RequestType            `protobuf:"varint,2,opt,name=type,proto3,enum=services.request.v1.RequestType" json:"type,omitempty"`
	GroupId       int64                  `protobuf:"varint,3,opt,name=group_id,json=groupId,proto3" json:"group_id,omitempty"`
	CategoryIds   []int64                `protobuf:"varint,4,rep,packed,name=category_ids,json=categoryIds,proto3" json:"category_ids,omitempty"`
	CityId        int64                  `protobuf:"varint,5,opt,name=city_id,json=cityId,proto3" json:"city_id,omitempty"`
	CarMakeId     int64                  `protobuf:"varint,6,opt,name=car_make_id,json=carMakeId,proto3" json:"car_make_id,omitempty"`
	CarModelId    int64                  `protobuf:"varint,7,opt,name=car_model_id,json=carModelId,proto3" json:"car_model_id,omitempty"`
	YearMin       int32                  `protobuf:"varint,8,opt,name=year_min,json=yearMin,proto3" json:"year_min,omitempty"`
	YearMax       int32                  `protobuf:"varint,9,opt,name=year_max,json=yearMax,proto3" json:"year_max,omitempty"`
	Page          int32                  `protobuf:"varint,10,opt,name=page,proto3" json:"page,omitempty"`
	PageSize      int32                  `protobuf:"varint,11,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchRequestsRequest) Reset() {
	*x = SearchRequestsRequest{}
	mi := &file_services_request_request_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchRequestsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchRequestsRequest) ProtoMessage() {}

func (x *SearchRequestsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchRequestsRequest.ProtoReflect.Descriptor instead.
func (*SearchRequestsRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{15}
}

func (x *SearchRequestsRequest) GetQuery() string {
	if x != nil {
		return x.Query
	}
	return ""
}

func (x *SearchRequestsRequest) GetType() RequestType {
	if x != nil {
		return x.Type
	}
	return RequestType_REQUEST_TYPE_UNSPECIFIED
}

func (x *SearchRequestsRequest) GetGroupId() int64 {
	if x != nil {
		return x.GroupId
	}
	return 0
}

func (x *SearchRequestsRequest) GetCategoryIds() []int64 {
	if x != nil {
		return x.CategoryIds
	}
	return nil
}

func (x *SearchRequestsRequest) GetCityId() int64 {
	if x != nil {
		return x.CityId
	}
	return 0
}

func (x *SearchRequestsRequest) GetCarMakeId() int64 {
	if x != nil {
		return x.CarMakeId
	}
	return 0
}

func (x *SearchRequestsRequest) GetCarModelId() int64 {
	if x != nil {
		return x.CarModelId
	}
	return 0
}

func (x *SearchRequestsRequest) GetYearMin() int32 {
	if x != nil {
		return x.YearMin
	}
	return 0
}

func (x *SearchRequestsRequest) GetYearMax() int32 {
	if x != nil {
		return x.YearMax
	}
	return 0
}

func (x *SearchRequestsRequest) GetPage() int32 {
	if x != nil {
		return x.Page
	}
	return 0
}

func (x *SearchRequestsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

type SearchRequestsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Requests      []*Request             `protobuf:"bytes,1,rep,name=requests,proto3" json:"requests,omitempty"`
	Total         int32                  `protobuf:"varint,2,opt,name=total,proto3" json:"total,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchRequestsResponse) Reset() {
	*x = SearchRequestsResponse{}
	mi := &file_services_request_request_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchRequestsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchRequestsResponse) ProtoMessage() {}

func (x *SearchRequestsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchRequestsResponse.ProtoReflect.Descriptor instead.
func (*SearchRequestsResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{16}
}

func (x *SearchRequestsResponse) GetRequests() []*Request {
	if x != nil {
		return x.Requests
	}
	return nil
}

func (x *SearchRequestsResponse) GetTotal() int32 {
	if x != nil {
		return x.Total
	}
	return 0
}

// ChangeStatus
type ChangeStatusRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID string
	Status        RequestStatus          `protobuf:"varint,2,opt,name=status,proto3,enum=services.request.v1.RequestStatus" json:"status,omitempty"`
	UserId        int64                  `protobuf:"varint,3,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeStatusRequest) Reset() {
	*x = ChangeStatusRequest{}
	mi := &file_services_request_request_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeStatusRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeStatusRequest) ProtoMessage() {}

func (x *ChangeStatusRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeStatusRequest.ProtoReflect.Descriptor instead.
func (*ChangeStatusRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{17}
}

func (x *ChangeStatusRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *ChangeStatusRequest) GetStatus() RequestStatus {
	if x != nil {
		return x.Status
	}
	return RequestStatus_REQUEST_STATUS_UNSPECIFIED
}

func (x *ChangeStatusRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

type ChangeStatusResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeStatusResponse) Reset() {
	*x = ChangeStatusResponse{}
	mi := &file_services_request_request_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeStatusResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeStatusResponse) ProtoMessage() {}

func (x *ChangeStatusResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeStatusResponse.ProtoReflect.Descriptor instead.
func (*ChangeStatusResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{18}
}

func (x *ChangeStatusResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

type PrepareRequestEscrowRequest struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	RequestId               string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	BidId                   int64                  `protobuf:"varint,2,opt,name=bid_id,json=bidId,proto3" json:"bid_id,omitempty"`
	PayerUserId             int64                  `protobuf:"varint,3,opt,name=payer_user_id,json=payerUserId,proto3" json:"payer_user_id,omitempty"`
	RecipientOrganizationId string                 `protobuf:"bytes,4,opt,name=recipient_organization_id,json=recipientOrganizationId,proto3" json:"recipient_organization_id,omitempty"`
	AmountMinor             int64                  `protobuf:"varint,5,opt,name=amount_minor,json=amountMinor,proto3" json:"amount_minor,omitempty"`
	Currency                string                 `protobuf:"bytes,6,opt,name=currency,proto3" json:"currency,omitempty"`
	IdempotencyKey          string                 `protobuf:"bytes,7,opt,name=idempotency_key,json=idempotencyKey,proto3" json:"idempotency_key,omitempty"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *PrepareRequestEscrowRequest) Reset() {
	*x = PrepareRequestEscrowRequest{}
	mi := &file_services_request_request_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PrepareRequestEscrowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PrepareRequestEscrowRequest) ProtoMessage() {}

func (x *PrepareRequestEscrowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PrepareRequestEscrowRequest.ProtoReflect.Descriptor instead.
func (*PrepareRequestEscrowRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{19}
}

func (x *PrepareRequestEscrowRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *PrepareRequestEscrowRequest) GetBidId() int64 {
	if x != nil {
		return x.BidId
	}
	return 0
}

func (x *PrepareRequestEscrowRequest) GetPayerUserId() int64 {
	if x != nil {
		return x.PayerUserId
	}
	return 0
}

func (x *PrepareRequestEscrowRequest) GetRecipientOrganizationId() string {
	if x != nil {
		return x.RecipientOrganizationId
	}
	return ""
}

func (x *PrepareRequestEscrowRequest) GetAmountMinor() int64 {
	if x != nil {
		return x.AmountMinor
	}
	return 0
}

func (x *PrepareRequestEscrowRequest) GetCurrency() string {
	if x != nil {
		return x.Currency
	}
	return ""
}

func (x *PrepareRequestEscrowRequest) GetIdempotencyKey() string {
	if x != nil {
		return x.IdempotencyKey
	}
	return ""
}

type PrepareRequestEscrowResponse struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	RequestId               string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	BidId                   int64                  `protobuf:"varint,2,opt,name=bid_id,json=bidId,proto3" json:"bid_id,omitempty"`
	PayerUserId             int64                  `protobuf:"varint,3,opt,name=payer_user_id,json=payerUserId,proto3" json:"payer_user_id,omitempty"`
	RecipientOrganizationId string                 `protobuf:"bytes,4,opt,name=recipient_organization_id,json=recipientOrganizationId,proto3" json:"recipient_organization_id,omitempty"`
	AmountMinor             int64                  `protobuf:"varint,5,opt,name=amount_minor,json=amountMinor,proto3" json:"amount_minor,omitempty"`
	Currency                string                 `protobuf:"bytes,6,opt,name=currency,proto3" json:"currency,omitempty"`
	State                   RequestEscrowState     `protobuf:"varint,7,opt,name=state,proto3,enum=services.request.v1.RequestEscrowState" json:"state,omitempty"`
	EverHeld                bool                   `protobuf:"varint,8,opt,name=ever_held,json=everHeld,proto3" json:"ever_held,omitempty"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *PrepareRequestEscrowResponse) Reset() {
	*x = PrepareRequestEscrowResponse{}
	mi := &file_services_request_request_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PrepareRequestEscrowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PrepareRequestEscrowResponse) ProtoMessage() {}

func (x *PrepareRequestEscrowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PrepareRequestEscrowResponse.ProtoReflect.Descriptor instead.
func (*PrepareRequestEscrowResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{20}
}

func (x *PrepareRequestEscrowResponse) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *PrepareRequestEscrowResponse) GetBidId() int64 {
	if x != nil {
		return x.BidId
	}
	return 0
}

func (x *PrepareRequestEscrowResponse) GetPayerUserId() int64 {
	if x != nil {
		return x.PayerUserId
	}
	return 0
}

func (x *PrepareRequestEscrowResponse) GetRecipientOrganizationId() string {
	if x != nil {
		return x.RecipientOrganizationId
	}
	return ""
}

func (x *PrepareRequestEscrowResponse) GetAmountMinor() int64 {
	if x != nil {
		return x.AmountMinor
	}
	return 0
}

func (x *PrepareRequestEscrowResponse) GetCurrency() string {
	if x != nil {
		return x.Currency
	}
	return ""
}

func (x *PrepareRequestEscrowResponse) GetState() RequestEscrowState {
	if x != nil {
		return x.State
	}
	return RequestEscrowState_REQUEST_ESCROW_STATE_UNSPECIFIED
}

func (x *PrepareRequestEscrowResponse) GetEverHeld() bool {
	if x != nil {
		return x.EverHeld
	}
	return false
}

type AuthorizeRequestEscrowCaptureRequest struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	RequestId               string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	BidId                   int64                  `protobuf:"varint,2,opt,name=bid_id,json=bidId,proto3" json:"bid_id,omitempty"`
	PayerUserId             int64                  `protobuf:"varint,3,opt,name=payer_user_id,json=payerUserId,proto3" json:"payer_user_id,omitempty"`
	RecipientOrganizationId string                 `protobuf:"bytes,4,opt,name=recipient_organization_id,json=recipientOrganizationId,proto3" json:"recipient_organization_id,omitempty"`
	AmountMinor             int64                  `protobuf:"varint,5,opt,name=amount_minor,json=amountMinor,proto3" json:"amount_minor,omitempty"`
	Currency                string                 `protobuf:"bytes,6,opt,name=currency,proto3" json:"currency,omitempty"`
	TransactionId           int64                  `protobuf:"varint,7,opt,name=transaction_id,json=transactionId,proto3" json:"transaction_id,omitempty"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *AuthorizeRequestEscrowCaptureRequest) Reset() {
	*x = AuthorizeRequestEscrowCaptureRequest{}
	mi := &file_services_request_request_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AuthorizeRequestEscrowCaptureRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AuthorizeRequestEscrowCaptureRequest) ProtoMessage() {}

func (x *AuthorizeRequestEscrowCaptureRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AuthorizeRequestEscrowCaptureRequest.ProtoReflect.Descriptor instead.
func (*AuthorizeRequestEscrowCaptureRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{21}
}

func (x *AuthorizeRequestEscrowCaptureRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *AuthorizeRequestEscrowCaptureRequest) GetBidId() int64 {
	if x != nil {
		return x.BidId
	}
	return 0
}

func (x *AuthorizeRequestEscrowCaptureRequest) GetPayerUserId() int64 {
	if x != nil {
		return x.PayerUserId
	}
	return 0
}

func (x *AuthorizeRequestEscrowCaptureRequest) GetRecipientOrganizationId() string {
	if x != nil {
		return x.RecipientOrganizationId
	}
	return ""
}

func (x *AuthorizeRequestEscrowCaptureRequest) GetAmountMinor() int64 {
	if x != nil {
		return x.AmountMinor
	}
	return 0
}

func (x *AuthorizeRequestEscrowCaptureRequest) GetCurrency() string {
	if x != nil {
		return x.Currency
	}
	return ""
}

func (x *AuthorizeRequestEscrowCaptureRequest) GetTransactionId() int64 {
	if x != nil {
		return x.TransactionId
	}
	return 0
}

type AuthorizeRequestEscrowCaptureResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Authorized    bool                   `protobuf:"varint,1,opt,name=authorized,proto3" json:"authorized,omitempty"`
	AlreadyHeld   bool                   `protobuf:"varint,2,opt,name=already_held,json=alreadyHeld,proto3" json:"already_held,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AuthorizeRequestEscrowCaptureResponse) Reset() {
	*x = AuthorizeRequestEscrowCaptureResponse{}
	mi := &file_services_request_request_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AuthorizeRequestEscrowCaptureResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AuthorizeRequestEscrowCaptureResponse) ProtoMessage() {}

func (x *AuthorizeRequestEscrowCaptureResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AuthorizeRequestEscrowCaptureResponse.ProtoReflect.Descriptor instead.
func (*AuthorizeRequestEscrowCaptureResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{22}
}

func (x *AuthorizeRequestEscrowCaptureResponse) GetAuthorized() bool {
	if x != nil {
		return x.Authorized
	}
	return false
}

func (x *AuthorizeRequestEscrowCaptureResponse) GetAlreadyHeld() bool {
	if x != nil {
		return x.AlreadyHeld
	}
	return false
}

type MarkRequestEscrowStateRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	BidId          int64                  `protobuf:"varint,2,opt,name=bid_id,json=bidId,proto3" json:"bid_id,omitempty"`
	TransactionId  int64                  `protobuf:"varint,3,opt,name=transaction_id,json=transactionId,proto3" json:"transaction_id,omitempty"`
	State          RequestEscrowState     `protobuf:"varint,4,opt,name=state,proto3,enum=services.request.v1.RequestEscrowState" json:"state,omitempty"`
	IdempotencyKey string                 `protobuf:"bytes,5,opt,name=idempotency_key,json=idempotencyKey,proto3" json:"idempotency_key,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *MarkRequestEscrowStateRequest) Reset() {
	*x = MarkRequestEscrowStateRequest{}
	mi := &file_services_request_request_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkRequestEscrowStateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkRequestEscrowStateRequest) ProtoMessage() {}

func (x *MarkRequestEscrowStateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkRequestEscrowStateRequest.ProtoReflect.Descriptor instead.
func (*MarkRequestEscrowStateRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{23}
}

func (x *MarkRequestEscrowStateRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *MarkRequestEscrowStateRequest) GetBidId() int64 {
	if x != nil {
		return x.BidId
	}
	return 0
}

func (x *MarkRequestEscrowStateRequest) GetTransactionId() int64 {
	if x != nil {
		return x.TransactionId
	}
	return 0
}

func (x *MarkRequestEscrowStateRequest) GetState() RequestEscrowState {
	if x != nil {
		return x.State
	}
	return RequestEscrowState_REQUEST_ESCROW_STATE_UNSPECIFIED
}

func (x *MarkRequestEscrowStateRequest) GetIdempotencyKey() string {
	if x != nil {
		return x.IdempotencyKey
	}
	return ""
}

type MarkRequestEscrowStateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	State         RequestEscrowState     `protobuf:"varint,1,opt,name=state,proto3,enum=services.request.v1.RequestEscrowState" json:"state,omitempty"`
	RequestClosed bool                   `protobuf:"varint,2,opt,name=request_closed,json=requestClosed,proto3" json:"request_closed,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MarkRequestEscrowStateResponse) Reset() {
	*x = MarkRequestEscrowStateResponse{}
	mi := &file_services_request_request_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkRequestEscrowStateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkRequestEscrowStateResponse) ProtoMessage() {}

func (x *MarkRequestEscrowStateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkRequestEscrowStateResponse.ProtoReflect.Descriptor instead.
func (*MarkRequestEscrowStateResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{24}
}

func (x *MarkRequestEscrowStateResponse) GetState() RequestEscrowState {
	if x != nil {
		return x.State
	}
	return RequestEscrowState_REQUEST_ESCROW_STATE_UNSPECIFIED
}

func (x *MarkRequestEscrowStateResponse) GetRequestClosed() bool {
	if x != nil {
		return x.RequestClosed
	}
	return false
}

type SelectRequestBidRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	BidId          int64                  `protobuf:"varint,2,opt,name=bid_id,json=bidId,proto3" json:"bid_id,omitempty"`
	PayerUserId    int64                  `protobuf:"varint,3,opt,name=payer_user_id,json=payerUserId,proto3" json:"payer_user_id,omitempty"`
	OrganizationId string                 `protobuf:"bytes,4,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *SelectRequestBidRequest) Reset() {
	*x = SelectRequestBidRequest{}
	mi := &file_services_request_request_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SelectRequestBidRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SelectRequestBidRequest) ProtoMessage() {}

func (x *SelectRequestBidRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SelectRequestBidRequest.ProtoReflect.Descriptor instead.
func (*SelectRequestBidRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{25}
}

func (x *SelectRequestBidRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *SelectRequestBidRequest) GetBidId() int64 {
	if x != nil {
		return x.BidId
	}
	return 0
}

func (x *SelectRequestBidRequest) GetPayerUserId() int64 {
	if x != nil {
		return x.PayerUserId
	}
	return 0
}

func (x *SelectRequestBidRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

type SelectRequestBidResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Selected        bool                   `protobuf:"varint,1,opt,name=selected,proto3" json:"selected,omitempty"`
	AlreadySelected bool                   `protobuf:"varint,2,opt,name=already_selected,json=alreadySelected,proto3" json:"already_selected,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *SelectRequestBidResponse) Reset() {
	*x = SelectRequestBidResponse{}
	mi := &file_services_request_request_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SelectRequestBidResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SelectRequestBidResponse) ProtoMessage() {}

func (x *SelectRequestBidResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SelectRequestBidResponse.ProtoReflect.Descriptor instead.
func (*SelectRequestBidResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{26}
}

func (x *SelectRequestBidResponse) GetSelected() bool {
	if x != nil {
		return x.Selected
	}
	return false
}

func (x *SelectRequestBidResponse) GetAlreadySelected() bool {
	if x != nil {
		return x.AlreadySelected
	}
	return false
}

type CloseRequestRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	UserId         int64                  `protobuf:"varint,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	IdempotencyKey string                 `protobuf:"bytes,3,opt,name=idempotency_key,json=idempotencyKey,proto3" json:"idempotency_key,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *CloseRequestRequest) Reset() {
	*x = CloseRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloseRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloseRequestRequest) ProtoMessage() {}

func (x *CloseRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloseRequestRequest.ProtoReflect.Descriptor instead.
func (*CloseRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{27}
}

func (x *CloseRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *CloseRequestRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *CloseRequestRequest) GetIdempotencyKey() string {
	if x != nil {
		return x.IdempotencyKey
	}
	return ""
}

type CloseRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CloseRequestResponse) Reset() {
	*x = CloseRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloseRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloseRequestResponse) ProtoMessage() {}

func (x *CloseRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloseRequestResponse.ProtoReflect.Descriptor instead.
func (*CloseRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{28}
}

func (x *CloseRequestResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

// GetUserRequests
type GetUserRequestsRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	UserId   int64                  `protobuf:"varint,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	Status   RequestStatus          `protobuf:"varint,2,opt,name=status,proto3,enum=services.request.v1.RequestStatus" json:"status,omitempty"`
	Page     int32                  `protobuf:"varint,3,opt,name=page,proto3" json:"page,omitempty"`
	PageSize int32                  `protobuf:"varint,4,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Multi-status filter (server-side IN); when non-empty it wins over `status`.
	// Enables correct tab bucketing (active = moderation+published+paused,
	// completed = deleted+closed) with correct pagination/meta — the old
	// single-status field forced clients/BFF to post-filter after pagination.
	Statuses      []RequestStatus `protobuf:"varint,5,rep,packed,name=statuses,proto3,enum=services.request.v1.RequestStatus" json:"statuses,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserRequestsRequest) Reset() {
	*x = GetUserRequestsRequest{}
	mi := &file_services_request_request_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRequestsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRequestsRequest) ProtoMessage() {}

func (x *GetUserRequestsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRequestsRequest.ProtoReflect.Descriptor instead.
func (*GetUserRequestsRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{29}
}

func (x *GetUserRequestsRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *GetUserRequestsRequest) GetStatus() RequestStatus {
	if x != nil {
		return x.Status
	}
	return RequestStatus_REQUEST_STATUS_UNSPECIFIED
}

func (x *GetUserRequestsRequest) GetPage() int32 {
	if x != nil {
		return x.Page
	}
	return 0
}

func (x *GetUserRequestsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *GetUserRequestsRequest) GetStatuses() []RequestStatus {
	if x != nil {
		return x.Statuses
	}
	return nil
}

type GetUserRequestsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Requests      []*Request             `protobuf:"bytes,1,rep,name=requests,proto3" json:"requests,omitempty"`
	Total         int32                  `protobuf:"varint,2,opt,name=total,proto3" json:"total,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserRequestsResponse) Reset() {
	*x = GetUserRequestsResponse{}
	mi := &file_services_request_request_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRequestsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRequestsResponse) ProtoMessage() {}

func (x *GetUserRequestsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRequestsResponse.ProtoReflect.Descriptor instead.
func (*GetUserRequestsResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{30}
}

func (x *GetUserRequestsResponse) GetRequests() []*Request {
	if x != nil {
		return x.Requests
	}
	return nil
}

func (x *GetUserRequestsResponse) GetTotal() int32 {
	if x != nil {
		return x.Total
	}
	return 0
}

// IncrementViews
type IncrementViewsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID string
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IncrementViewsRequest) Reset() {
	*x = IncrementViewsRequest{}
	mi := &file_services_request_request_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IncrementViewsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IncrementViewsRequest) ProtoMessage() {}

func (x *IncrementViewsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IncrementViewsRequest.ProtoReflect.Descriptor instead.
func (*IncrementViewsRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{31}
}

func (x *IncrementViewsRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

type IncrementViewsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IncrementViewsResponse) Reset() {
	*x = IncrementViewsResponse{}
	mi := &file_services_request_request_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IncrementViewsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IncrementViewsResponse) ProtoMessage() {}

func (x *IncrementViewsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IncrementViewsResponse.ProtoReflect.Descriptor instead.
func (*IncrementViewsResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{32}
}

func (x *IncrementViewsResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

// GetSuggestions
type GetSuggestionsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         string                 `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	Limit         int32                  `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSuggestionsRequest) Reset() {
	*x = GetSuggestionsRequest{}
	mi := &file_services_request_request_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSuggestionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSuggestionsRequest) ProtoMessage() {}

func (x *GetSuggestionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSuggestionsRequest.ProtoReflect.Descriptor instead.
func (*GetSuggestionsRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{33}
}

func (x *GetSuggestionsRequest) GetQuery() string {
	if x != nil {
		return x.Query
	}
	return ""
}

func (x *GetSuggestionsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

type GetSuggestionsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Suggestions   []string               `protobuf:"bytes,1,rep,name=suggestions,proto3" json:"suggestions,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSuggestionsResponse) Reset() {
	*x = GetSuggestionsResponse{}
	mi := &file_services_request_request_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSuggestionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSuggestionsResponse) ProtoMessage() {}

func (x *GetSuggestionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSuggestionsResponse.ProtoReflect.Descriptor instead.
func (*GetSuggestionsResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{34}
}

func (x *GetSuggestionsResponse) GetSuggestions() []string {
	if x != nil {
		return x.Suggestions
	}
	return nil
}

// GetNewRequestsForOrganization
type GetNewRequestsForOrganizationRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	OrganizationId string                 `protobuf:"bytes,1,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"` // UUID from organization-service
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetNewRequestsForOrganizationRequest) Reset() {
	*x = GetNewRequestsForOrganizationRequest{}
	mi := &file_services_request_request_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNewRequestsForOrganizationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNewRequestsForOrganizationRequest) ProtoMessage() {}

func (x *GetNewRequestsForOrganizationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNewRequestsForOrganizationRequest.ProtoReflect.Descriptor instead.
func (*GetNewRequestsForOrganizationRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{35}
}

func (x *GetNewRequestsForOrganizationRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

type GetNewRequestsForOrganizationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestIds    []string               `protobuf:"bytes,1,rep,name=request_ids,json=requestIds,proto3" json:"request_ids,omitempty"` // UUID strings
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNewRequestsForOrganizationResponse) Reset() {
	*x = GetNewRequestsForOrganizationResponse{}
	mi := &file_services_request_request_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNewRequestsForOrganizationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNewRequestsForOrganizationResponse) ProtoMessage() {}

func (x *GetNewRequestsForOrganizationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNewRequestsForOrganizationResponse.ProtoReflect.Descriptor instead.
func (*GetNewRequestsForOrganizationResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{36}
}

func (x *GetNewRequestsForOrganizationResponse) GetRequestIds() []string {
	if x != nil {
		return x.RequestIds
	}
	return nil
}

// MarkRequestAsViewed
type MarkRequestAsViewedRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`                // UUID string
	OrganizationId string                 `protobuf:"bytes,2,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"` // UUID from organization-service
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *MarkRequestAsViewedRequest) Reset() {
	*x = MarkRequestAsViewedRequest{}
	mi := &file_services_request_request_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkRequestAsViewedRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkRequestAsViewedRequest) ProtoMessage() {}

func (x *MarkRequestAsViewedRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkRequestAsViewedRequest.ProtoReflect.Descriptor instead.
func (*MarkRequestAsViewedRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{37}
}

func (x *MarkRequestAsViewedRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *MarkRequestAsViewedRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

type MarkRequestAsViewedResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MarkRequestAsViewedResponse) Reset() {
	*x = MarkRequestAsViewedResponse{}
	mi := &file_services_request_request_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkRequestAsViewedResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkRequestAsViewedResponse) ProtoMessage() {}

func (x *MarkRequestAsViewedResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkRequestAsViewedResponse.ProtoReflect.Descriptor instead.
func (*MarkRequestAsViewedResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{38}
}

func (x *MarkRequestAsViewedResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

// IsRequestNew
type IsRequestNewRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`                // UUID string
	OrganizationId string                 `protobuf:"bytes,2,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"` // UUID from organization-service
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *IsRequestNewRequest) Reset() {
	*x = IsRequestNewRequest{}
	mi := &file_services_request_request_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IsRequestNewRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IsRequestNewRequest) ProtoMessage() {}

func (x *IsRequestNewRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IsRequestNewRequest.ProtoReflect.Descriptor instead.
func (*IsRequestNewRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{39}
}

func (x *IsRequestNewRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *IsRequestNewRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

type IsRequestNewResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IsNew         bool                   `protobuf:"varint,1,opt,name=is_new,json=isNew,proto3" json:"is_new,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IsRequestNewResponse) Reset() {
	*x = IsRequestNewResponse{}
	mi := &file_services_request_request_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IsRequestNewResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IsRequestNewResponse) ProtoMessage() {}

func (x *IsRequestNewResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IsRequestNewResponse.ProtoReflect.Descriptor instead.
func (*IsRequestNewResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{40}
}

func (x *IsRequestNewResponse) GetIsNew() bool {
	if x != nil {
		return x.IsNew
	}
	return false
}

// DismissRequest — hide a request for an organization ("не интересно").
// Wraps domain DismissRequest(ctx, requestID, orgID).
type DismissRequestRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`                // UUID string
	OrganizationId string                 `protobuf:"bytes,2,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"` // UUID from organization-service
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *DismissRequestRequest) Reset() {
	*x = DismissRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[41]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DismissRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DismissRequestRequest) ProtoMessage() {}

func (x *DismissRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[41]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DismissRequestRequest.ProtoReflect.Descriptor instead.
func (*DismissRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{41}
}

func (x *DismissRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *DismissRequestRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

type DismissRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DismissRequestResponse) Reset() {
	*x = DismissRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[42]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DismissRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DismissRequestResponse) ProtoMessage() {}

func (x *DismissRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[42]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DismissRequestResponse.ProtoReflect.Descriptor instead.
func (*DismissRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{42}
}

func (x *DismissRequestResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

// CountUnreadForOrganization — count unread & not-dismissed requests for an
// organization, scoped by the org-profile filter. Wraps domain
// CountUnreadForOrganization(ctx, orgID, model.RequestFilter).
//
// Filter fields map 1:1 to model.RequestFilter. Empty/zero = no filter on that
// field. Note the domain filter carries a single category_id (not a list).
type CountUnreadForOrganizationRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	OrganizationId string                 `protobuf:"bytes,1,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"` // UUID from organization-service
	// Org-profile filter (subset of model.RequestFilter used by the feed).
	Type          RequestType `protobuf:"varint,2,opt,name=type,proto3,enum=services.request.v1.RequestType" json:"type,omitempty"` // UNSPECIFIED = any type
	GroupId       int64       `protobuf:"varint,3,opt,name=group_id,json=groupId,proto3" json:"group_id,omitempty"`                 // 0 = any group
	CategoryId    int64       `protobuf:"varint,4,opt,name=category_id,json=categoryId,proto3" json:"category_id,omitempty"`        // 0 = any category (model.RequestFilter.CategoryID)
	CityId        int64       `protobuf:"varint,5,opt,name=city_id,json=cityId,proto3" json:"city_id,omitempty"`                    // 0 = any city
	CarMakeId     int64       `protobuf:"varint,6,opt,name=car_make_id,json=carMakeId,proto3" json:"car_make_id,omitempty"`         // 0 = any make
	CarModelId    int64       `protobuf:"varint,7,opt,name=car_model_id,json=carModelId,proto3" json:"car_model_id,omitempty"`      // 0 = any model
	Year          int32       `protobuf:"varint,8,opt,name=year,proto3" json:"year,omitempty"`                                      // 0 = any year
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CountUnreadForOrganizationRequest) Reset() {
	*x = CountUnreadForOrganizationRequest{}
	mi := &file_services_request_request_proto_msgTypes[43]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CountUnreadForOrganizationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CountUnreadForOrganizationRequest) ProtoMessage() {}

func (x *CountUnreadForOrganizationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[43]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CountUnreadForOrganizationRequest.ProtoReflect.Descriptor instead.
func (*CountUnreadForOrganizationRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{43}
}

func (x *CountUnreadForOrganizationRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

func (x *CountUnreadForOrganizationRequest) GetType() RequestType {
	if x != nil {
		return x.Type
	}
	return RequestType_REQUEST_TYPE_UNSPECIFIED
}

func (x *CountUnreadForOrganizationRequest) GetGroupId() int64 {
	if x != nil {
		return x.GroupId
	}
	return 0
}

func (x *CountUnreadForOrganizationRequest) GetCategoryId() int64 {
	if x != nil {
		return x.CategoryId
	}
	return 0
}

func (x *CountUnreadForOrganizationRequest) GetCityId() int64 {
	if x != nil {
		return x.CityId
	}
	return 0
}

func (x *CountUnreadForOrganizationRequest) GetCarMakeId() int64 {
	if x != nil {
		return x.CarMakeId
	}
	return 0
}

func (x *CountUnreadForOrganizationRequest) GetCarModelId() int64 {
	if x != nil {
		return x.CarModelId
	}
	return 0
}

func (x *CountUnreadForOrganizationRequest) GetYear() int32 {
	if x != nil {
		return x.Year
	}
	return 0
}

type CountUnreadForOrganizationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Count         int64                  `protobuf:"varint,1,opt,name=count,proto3" json:"count,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CountUnreadForOrganizationResponse) Reset() {
	*x = CountUnreadForOrganizationResponse{}
	mi := &file_services_request_request_proto_msgTypes[44]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CountUnreadForOrganizationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CountUnreadForOrganizationResponse) ProtoMessage() {}

func (x *CountUnreadForOrganizationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[44]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CountUnreadForOrganizationResponse.ProtoReflect.Descriptor instead.
func (*CountUnreadForOrganizationResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{44}
}

func (x *CountUnreadForOrganizationResponse) GetCount() int64 {
	if x != nil {
		return x.Count
	}
	return 0
}

// ClassifyRequest — AI auto-classification of repair/parts requests
type ClassifyRequestRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`               // UUID of the request to classify
	GroupId       int64                  `protobuf:"varint,2,opt,name=group_id,json=groupId,proto3" json:"group_id,omitempty"`                    // Service group (1=STO/Repair, 2=Bodyshop)
	CategoryIds   []int64                `protobuf:"varint,3,rep,packed,name=category_ids,json=categoryIds,proto3" json:"category_ids,omitempty"` // Service categories
	CleanedNote   string                 `protobuf:"bytes,4,opt,name=cleaned_note,json=cleanedNote,proto3" json:"cleaned_note,omitempty"`         // Moderated/cleaned description text
	Publish       bool                   `protobuf:"varint,5,opt,name=publish,proto3" json:"publish,omitempty"`                                   // If true, also transition status to PUBLISHED
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ClassifyRequestRequest) Reset() {
	*x = ClassifyRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[45]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClassifyRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClassifyRequestRequest) ProtoMessage() {}

func (x *ClassifyRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[45]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClassifyRequestRequest.ProtoReflect.Descriptor instead.
func (*ClassifyRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{45}
}

func (x *ClassifyRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *ClassifyRequestRequest) GetGroupId() int64 {
	if x != nil {
		return x.GroupId
	}
	return 0
}

func (x *ClassifyRequestRequest) GetCategoryIds() []int64 {
	if x != nil {
		return x.CategoryIds
	}
	return nil
}

func (x *ClassifyRequestRequest) GetCleanedNote() string {
	if x != nil {
		return x.CleanedNote
	}
	return ""
}

func (x *ClassifyRequestRequest) GetPublish() bool {
	if x != nil {
		return x.Publish
	}
	return false
}

type ClassifyRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ClassifyRequestResponse) Reset() {
	*x = ClassifyRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[46]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClassifyRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClassifyRequestResponse) ProtoMessage() {}

func (x *ClassifyRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[46]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClassifyRequestResponse.ProtoReflect.Descriptor instead.
func (*ClassifyRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{46}
}

func (x *ClassifyRequestResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

type GetRequestForClassificationRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRequestForClassificationRequest) Reset() {
	*x = GetRequestForClassificationRequest{}
	mi := &file_services_request_request_proto_msgTypes[47]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRequestForClassificationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRequestForClassificationRequest) ProtoMessage() {}

func (x *GetRequestForClassificationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[47]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRequestForClassificationRequest.ProtoReflect.Descriptor instead.
func (*GetRequestForClassificationRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{47}
}

func (x *GetRequestForClassificationRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

type GetRequestForClassificationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Note          string                 `protobuf:"bytes,1,opt,name=note,proto3" json:"note,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRequestForClassificationResponse) Reset() {
	*x = GetRequestForClassificationResponse{}
	mi := &file_services_request_request_proto_msgTypes[48]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRequestForClassificationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRequestForClassificationResponse) ProtoMessage() {}

func (x *GetRequestForClassificationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[48]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRequestForClassificationResponse.ProtoReflect.Descriptor instead.
func (*GetRequestForClassificationResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{48}
}

func (x *GetRequestForClassificationResponse) GetNote() string {
	if x != nil {
		return x.Note
	}
	return ""
}

type GetInsurancePayoutTermsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetInsurancePayoutTermsRequest) Reset() {
	*x = GetInsurancePayoutTermsRequest{}
	mi := &file_services_request_request_proto_msgTypes[49]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetInsurancePayoutTermsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetInsurancePayoutTermsRequest) ProtoMessage() {}

func (x *GetInsurancePayoutTermsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[49]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetInsurancePayoutTermsRequest.ProtoReflect.Descriptor instead.
func (*GetInsurancePayoutTermsRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{49}
}

func (x *GetInsurancePayoutTermsRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

type GetInsurancePayoutTermsResponse struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	RequestId         string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Kind              RequestKind            `protobuf:"varint,2,opt,name=kind,proto3,enum=services.request.v1.RequestKind" json:"kind,omitempty"`
	Status            RequestStatus          `protobuf:"varint,3,opt,name=status,proto3,enum=services.request.v1.RequestStatus" json:"status,omitempty"`
	PayoutAmountMinor int64                  `protobuf:"varint,4,opt,name=payout_amount_minor,json=payoutAmountMinor,proto3" json:"payout_amount_minor,omitempty"`
	AssignmentMode    AssignmentMode         `protobuf:"varint,5,opt,name=assignment_mode,json=assignmentMode,proto3,enum=services.request.v1.AssignmentMode" json:"assignment_mode,omitempty"`
	ClaimedByOrgId    string                 `protobuf:"bytes,6,opt,name=claimed_by_org_id,json=claimedByOrgId,proto3" json:"claimed_by_org_id,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *GetInsurancePayoutTermsResponse) Reset() {
	*x = GetInsurancePayoutTermsResponse{}
	mi := &file_services_request_request_proto_msgTypes[50]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetInsurancePayoutTermsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetInsurancePayoutTermsResponse) ProtoMessage() {}

func (x *GetInsurancePayoutTermsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[50]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetInsurancePayoutTermsResponse.ProtoReflect.Descriptor instead.
func (*GetInsurancePayoutTermsResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{50}
}

func (x *GetInsurancePayoutTermsResponse) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *GetInsurancePayoutTermsResponse) GetKind() RequestKind {
	if x != nil {
		return x.Kind
	}
	return RequestKind_REQUEST_KIND_UNSPECIFIED
}

func (x *GetInsurancePayoutTermsResponse) GetStatus() RequestStatus {
	if x != nil {
		return x.Status
	}
	return RequestStatus_REQUEST_STATUS_UNSPECIFIED
}

func (x *GetInsurancePayoutTermsResponse) GetPayoutAmountMinor() int64 {
	if x != nil {
		return x.PayoutAmountMinor
	}
	return 0
}

func (x *GetInsurancePayoutTermsResponse) GetAssignmentMode() AssignmentMode {
	if x != nil {
		return x.AssignmentMode
	}
	return AssignmentMode_ASSIGNMENT_MODE_UNSPECIFIED
}

func (x *GetInsurancePayoutTermsResponse) GetClaimedByOrgId() string {
	if x != nil {
		return x.ClaimedByOrgId
	}
	return ""
}

type GetRequestEligibilityInfoRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRequestEligibilityInfoRequest) Reset() {
	*x = GetRequestEligibilityInfoRequest{}
	mi := &file_services_request_request_proto_msgTypes[51]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRequestEligibilityInfoRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRequestEligibilityInfoRequest) ProtoMessage() {}

func (x *GetRequestEligibilityInfoRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[51]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRequestEligibilityInfoRequest.ProtoReflect.Descriptor instead.
func (*GetRequestEligibilityInfoRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{51}
}

func (x *GetRequestEligibilityInfoRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

type GetRequestEligibilityInfoResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        int64                  `protobuf:"varint,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	Type          RequestType            `protobuf:"varint,2,opt,name=type,proto3,enum=services.request.v1.RequestType" json:"type,omitempty"`
	RepairOrderId *int64                 `protobuf:"varint,3,opt,name=repair_order_id,json=repairOrderId,proto3,oneof" json:"repair_order_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRequestEligibilityInfoResponse) Reset() {
	*x = GetRequestEligibilityInfoResponse{}
	mi := &file_services_request_request_proto_msgTypes[52]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRequestEligibilityInfoResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRequestEligibilityInfoResponse) ProtoMessage() {}

func (x *GetRequestEligibilityInfoResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[52]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRequestEligibilityInfoResponse.ProtoReflect.Descriptor instead.
func (*GetRequestEligibilityInfoResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{52}
}

func (x *GetRequestEligibilityInfoResponse) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *GetRequestEligibilityInfoResponse) GetType() RequestType {
	if x != nil {
		return x.Type
	}
	return RequestType_REQUEST_TYPE_UNSPECIFIED
}

func (x *GetRequestEligibilityInfoResponse) GetRepairOrderId() int64 {
	if x != nil && x.RepairOrderId != nil {
		return *x.RepairOrderId
	}
	return 0
}

// GetUserRequestCounts — per-state counts of a user's requests (legacy ads/count).
type GetUserRequestCountsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        int64                  `protobuf:"varint,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	Type          RequestType            `protobuf:"varint,2,opt,name=type,proto3,enum=services.request.v1.RequestType" json:"type,omitempty"` // optional filter; 0 (unspecified) = all types
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserRequestCountsRequest) Reset() {
	*x = GetUserRequestCountsRequest{}
	mi := &file_services_request_request_proto_msgTypes[53]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRequestCountsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRequestCountsRequest) ProtoMessage() {}

func (x *GetUserRequestCountsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[53]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRequestCountsRequest.ProtoReflect.Descriptor instead.
func (*GetUserRequestCountsRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{53}
}

func (x *GetUserRequestCountsRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *GetUserRequestCountsRequest) GetType() RequestType {
	if x != nil {
		return x.Type
	}
	return RequestType_REQUEST_TYPE_UNSPECIFIED
}

type GetUserRequestCountsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Total         int32                  `protobuf:"varint,1,opt,name=total,proto3" json:"total,omitempty"`
	Active        int32                  `protobuf:"varint,2,opt,name=active,proto3" json:"active,omitempty"`         // REQUEST_STATUS_PUBLISHED
	Moderation    int32                  `protobuf:"varint,3,opt,name=moderation,proto3" json:"moderation,omitempty"` // REQUEST_STATUS_MODERATION
	Inactive      int32                  `protobuf:"varint,4,opt,name=inactive,proto3" json:"inactive,omitempty"`     // REQUEST_STATUS_DELETED + REQUEST_STATUS_CLOSED
	Paused        int32                  `protobuf:"varint,5,opt,name=paused,proto3" json:"paused,omitempty"`         // REQUEST_STATUS_PAUSED (separate bucket)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserRequestCountsResponse) Reset() {
	*x = GetUserRequestCountsResponse{}
	mi := &file_services_request_request_proto_msgTypes[54]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRequestCountsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRequestCountsResponse) ProtoMessage() {}

func (x *GetUserRequestCountsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[54]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRequestCountsResponse.ProtoReflect.Descriptor instead.
func (*GetUserRequestCountsResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{54}
}

func (x *GetUserRequestCountsResponse) GetTotal() int32 {
	if x != nil {
		return x.Total
	}
	return 0
}

func (x *GetUserRequestCountsResponse) GetActive() int32 {
	if x != nil {
		return x.Active
	}
	return 0
}

func (x *GetUserRequestCountsResponse) GetModeration() int32 {
	if x != nil {
		return x.Moderation
	}
	return 0
}

func (x *GetUserRequestCountsResponse) GetInactive() int32 {
	if x != nil {
		return x.Inactive
	}
	return 0
}

func (x *GetUserRequestCountsResponse) GetPaused() int32 {
	if x != nil {
		return x.Paused
	}
	return 0
}

// PauseRequest — hide a published request from the СТО feed.
type PauseRequestRequest struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	RequestId string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID of the request to pause
	UserId    int64                  `protobuf:"varint,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`         // must match request.user_id
	// Optional UUID of the organization the customer is pausing for.
	// Mirrors legacy paused_by_service_id. Empty = self-pause.
	PausedByOrgId *string `protobuf:"bytes,3,opt,name=paused_by_org_id,json=pausedByOrgId,proto3,oneof" json:"paused_by_org_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PauseRequestRequest) Reset() {
	*x = PauseRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[55]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PauseRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PauseRequestRequest) ProtoMessage() {}

func (x *PauseRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[55]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PauseRequestRequest.ProtoReflect.Descriptor instead.
func (*PauseRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{55}
}

func (x *PauseRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *PauseRequestRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *PauseRequestRequest) GetPausedByOrgId() string {
	if x != nil && x.PausedByOrgId != nil {
		return *x.PausedByOrgId
	}
	return ""
}

type PauseRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PauseRequestResponse) Reset() {
	*x = PauseRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[56]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PauseRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PauseRequestResponse) ProtoMessage() {}

func (x *PauseRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[56]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PauseRequestResponse.ProtoReflect.Descriptor instead.
func (*PauseRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{56}
}

func (x *PauseRequestResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

// UnpauseRequest — restore a paused request to PUBLISHED.
type UnpauseRequestRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID of the request to unpause
	UserId        int64                  `protobuf:"varint,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`         // must match request.user_id
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UnpauseRequestRequest) Reset() {
	*x = UnpauseRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[57]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnpauseRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnpauseRequestRequest) ProtoMessage() {}

func (x *UnpauseRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[57]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnpauseRequestRequest.ProtoReflect.Descriptor instead.
func (*UnpauseRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{57}
}

func (x *UnpauseRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *UnpauseRequestRequest) GetUserId() int64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

type UnpauseRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UnpauseRequestResponse) Reset() {
	*x = UnpauseRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[58]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnpauseRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnpauseRequestResponse) ProtoMessage() {}

func (x *UnpauseRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[58]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnpauseRequestResponse.ProtoReflect.Descriptor instead.
func (*UnpauseRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{58}
}

func (x *UnpauseRequestResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

// InsuranceInfoInput carries the admin-only data supplied when creating an
// insurance request: the price the client pays (X), the insurer and client
// contacts, plus the targeting-criteria snapshot kept for audit/history.
type InsuranceInfoInput struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	InsurancePrice   int64                  `protobuf:"varint,1,opt,name=insurance_price,json=insurancePrice,proto3" json:"insurance_price,omitempty"` // X, тиыны — админ-only
	InsuranceCompany string                 `protobuf:"bytes,2,opt,name=insurance_company,json=insuranceCompany,proto3" json:"insurance_company,omitempty"`
	ClientName       string                 `protobuf:"bytes,3,opt,name=client_name,json=clientName,proto3" json:"client_name,omitempty"`
	ClientPhone      string                 `protobuf:"bytes,4,opt,name=client_phone,json=clientPhone,proto3" json:"client_phone,omitempty"`
	MinRating        float64                `protobuf:"fixed64,5,opt,name=min_rating,json=minRating,proto3" json:"min_rating,omitempty"` // критерии snapshot'а (для истории/аудита)
	MinRatersCount   int32                  `protobuf:"varint,6,opt,name=min_raters_count,json=minRatersCount,proto3" json:"min_raters_count,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *InsuranceInfoInput) Reset() {
	*x = InsuranceInfoInput{}
	mi := &file_services_request_request_proto_msgTypes[59]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *InsuranceInfoInput) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*InsuranceInfoInput) ProtoMessage() {}

func (x *InsuranceInfoInput) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[59]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use InsuranceInfoInput.ProtoReflect.Descriptor instead.
func (*InsuranceInfoInput) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{59}
}

func (x *InsuranceInfoInput) GetInsurancePrice() int64 {
	if x != nil {
		return x.InsurancePrice
	}
	return 0
}

func (x *InsuranceInfoInput) GetInsuranceCompany() string {
	if x != nil {
		return x.InsuranceCompany
	}
	return ""
}

func (x *InsuranceInfoInput) GetClientName() string {
	if x != nil {
		return x.ClientName
	}
	return ""
}

func (x *InsuranceInfoInput) GetClientPhone() string {
	if x != nil {
		return x.ClientPhone
	}
	return ""
}

func (x *InsuranceInfoInput) GetMinRating() float64 {
	if x != nil {
		return x.MinRating
	}
	return 0
}

func (x *InsuranceInfoInput) GetMinRatersCount() int32 {
	if x != nil {
		return x.MinRatersCount
	}
	return 0
}

// RequestInsuranceInfo is the persisted admin-only projection returned by
// GetRequestInsuranceInfo: client price (X), contacts, the targeting snapshot
// and the winning org once claimed.
type RequestInsuranceInfo struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	RequestId        string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	InsurancePrice   int64                  `protobuf:"varint,2,opt,name=insurance_price,json=insurancePrice,proto3" json:"insurance_price,omitempty"`
	InsuranceCompany string                 `protobuf:"bytes,3,opt,name=insurance_company,json=insuranceCompany,proto3" json:"insurance_company,omitempty"`
	ClientName       string                 `protobuf:"bytes,4,opt,name=client_name,json=clientName,proto3" json:"client_name,omitempty"`
	ClientPhone      string                 `protobuf:"bytes,5,opt,name=client_phone,json=clientPhone,proto3" json:"client_phone,omitempty"`
	MinRating        float64                `protobuf:"fixed64,6,opt,name=min_rating,json=minRating,proto3" json:"min_rating,omitempty"`
	MinRatersCount   int32                  `protobuf:"varint,7,opt,name=min_raters_count,json=minRatersCount,proto3" json:"min_raters_count,omitempty"`
	TargetOrgIds     []string               `protobuf:"bytes,8,rep,name=target_org_ids,json=targetOrgIds,proto3" json:"target_org_ids,omitempty"`
	ClaimedByOrgId   *string                `protobuf:"bytes,9,opt,name=claimed_by_org_id,json=claimedByOrgId,proto3,oneof" json:"claimed_by_org_id,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *RequestInsuranceInfo) Reset() {
	*x = RequestInsuranceInfo{}
	mi := &file_services_request_request_proto_msgTypes[60]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RequestInsuranceInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RequestInsuranceInfo) ProtoMessage() {}

func (x *RequestInsuranceInfo) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[60]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RequestInsuranceInfo.ProtoReflect.Descriptor instead.
func (*RequestInsuranceInfo) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{60}
}

func (x *RequestInsuranceInfo) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *RequestInsuranceInfo) GetInsurancePrice() int64 {
	if x != nil {
		return x.InsurancePrice
	}
	return 0
}

func (x *RequestInsuranceInfo) GetInsuranceCompany() string {
	if x != nil {
		return x.InsuranceCompany
	}
	return ""
}

func (x *RequestInsuranceInfo) GetClientName() string {
	if x != nil {
		return x.ClientName
	}
	return ""
}

func (x *RequestInsuranceInfo) GetClientPhone() string {
	if x != nil {
		return x.ClientPhone
	}
	return ""
}

func (x *RequestInsuranceInfo) GetMinRating() float64 {
	if x != nil {
		return x.MinRating
	}
	return 0
}

func (x *RequestInsuranceInfo) GetMinRatersCount() int32 {
	if x != nil {
		return x.MinRatersCount
	}
	return 0
}

func (x *RequestInsuranceInfo) GetTargetOrgIds() []string {
	if x != nil {
		return x.TargetOrgIds
	}
	return nil
}

func (x *RequestInsuranceInfo) GetClaimedByOrgId() string {
	if x != nil && x.ClaimedByOrgId != nil {
		return *x.ClaimedByOrgId
	}
	return ""
}

// GetRequestInsuranceInfo
type GetRequestInsuranceInfoRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID string
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRequestInsuranceInfoRequest) Reset() {
	*x = GetRequestInsuranceInfoRequest{}
	mi := &file_services_request_request_proto_msgTypes[61]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRequestInsuranceInfoRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRequestInsuranceInfoRequest) ProtoMessage() {}

func (x *GetRequestInsuranceInfoRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[61]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRequestInsuranceInfoRequest.ProtoReflect.Descriptor instead.
func (*GetRequestInsuranceInfoRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{61}
}

func (x *GetRequestInsuranceInfoRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

type GetRequestInsuranceInfoResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Info          *RequestInsuranceInfo  `protobuf:"bytes,1,opt,name=info,proto3" json:"info,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRequestInsuranceInfoResponse) Reset() {
	*x = GetRequestInsuranceInfoResponse{}
	mi := &file_services_request_request_proto_msgTypes[62]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRequestInsuranceInfoResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRequestInsuranceInfoResponse) ProtoMessage() {}

func (x *GetRequestInsuranceInfoResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[62]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRequestInsuranceInfoResponse.ProtoReflect.Descriptor instead.
func (*GetRequestInsuranceInfoResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{62}
}

func (x *GetRequestInsuranceInfoResponse) GetInfo() *RequestInsuranceInfo {
	if x != nil {
		return x.Info
	}
	return nil
}

// ClaimInsuranceRequest
type ClaimInsuranceRequestRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID string
	OrgId         string                 `protobuf:"bytes,2,opt,name=org_id,json=orgId,proto3" json:"org_id,omitempty"`             // UUID from organization-service
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ClaimInsuranceRequestRequest) Reset() {
	*x = ClaimInsuranceRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[63]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClaimInsuranceRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClaimInsuranceRequestRequest) ProtoMessage() {}

func (x *ClaimInsuranceRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[63]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClaimInsuranceRequestRequest.ProtoReflect.Descriptor instead.
func (*ClaimInsuranceRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{63}
}

func (x *ClaimInsuranceRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *ClaimInsuranceRequestRequest) GetOrgId() string {
	if x != nil {
		return x.OrgId
	}
	return ""
}

type ClaimInsuranceRequestResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Claimed        bool                   `protobuf:"varint,1,opt,name=claimed,proto3" json:"claimed,omitempty"`
	ClaimedByOrgId string                 `protobuf:"bytes,2,opt,name=claimed_by_org_id,json=claimedByOrgId,proto3" json:"claimed_by_org_id,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ClaimInsuranceRequestResponse) Reset() {
	*x = ClaimInsuranceRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[64]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClaimInsuranceRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClaimInsuranceRequestResponse) ProtoMessage() {}

func (x *ClaimInsuranceRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[64]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClaimInsuranceRequestResponse.ProtoReflect.Descriptor instead.
func (*ClaimInsuranceRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{64}
}

func (x *ClaimInsuranceRequestResponse) GetClaimed() bool {
	if x != nil {
		return x.Claimed
	}
	return false
}

func (x *ClaimInsuranceRequestResponse) GetClaimedByOrgId() string {
	if x != nil {
		return x.ClaimedByOrgId
	}
	return ""
}

// IsOrgTargeted
type IsOrgTargetedRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // UUID string
	OrgId         string                 `protobuf:"bytes,2,opt,name=org_id,json=orgId,proto3" json:"org_id,omitempty"`             // UUID from organization-service
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IsOrgTargetedRequest) Reset() {
	*x = IsOrgTargetedRequest{}
	mi := &file_services_request_request_proto_msgTypes[65]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IsOrgTargetedRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IsOrgTargetedRequest) ProtoMessage() {}

func (x *IsOrgTargetedRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[65]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IsOrgTargetedRequest.ProtoReflect.Descriptor instead.
func (*IsOrgTargetedRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{65}
}

func (x *IsOrgTargetedRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *IsOrgTargetedRequest) GetOrgId() string {
	if x != nil {
		return x.OrgId
	}
	return ""
}

type IsOrgTargetedResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Targeted      bool                   `protobuf:"varint,1,opt,name=targeted,proto3" json:"targeted,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IsOrgTargetedResponse) Reset() {
	*x = IsOrgTargetedResponse{}
	mi := &file_services_request_request_proto_msgTypes[66]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IsOrgTargetedResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IsOrgTargetedResponse) ProtoMessage() {}

func (x *IsOrgTargetedResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[66]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IsOrgTargetedResponse.ProtoReflect.Descriptor instead.
func (*IsOrgTargetedResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{66}
}

func (x *IsOrgTargetedResponse) GetTargeted() bool {
	if x != nil {
		return x.Targeted
	}
	return false
}

// CompleteInsuranceRequest
type CompleteInsuranceRequestRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`                // UUID string
	OrganizationId string                 `protobuf:"bytes,2,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"` // must equal the winning claimed organization
	IdempotencyKey string                 `protobuf:"bytes,3,opt,name=idempotency_key,json=idempotencyKey,proto3" json:"idempotency_key,omitempty"` // caller-stable, scoped to this request/action
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *CompleteInsuranceRequestRequest) Reset() {
	*x = CompleteInsuranceRequestRequest{}
	mi := &file_services_request_request_proto_msgTypes[67]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CompleteInsuranceRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CompleteInsuranceRequestRequest) ProtoMessage() {}

func (x *CompleteInsuranceRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[67]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CompleteInsuranceRequestRequest.ProtoReflect.Descriptor instead.
func (*CompleteInsuranceRequestRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{67}
}

func (x *CompleteInsuranceRequestRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *CompleteInsuranceRequestRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

func (x *CompleteInsuranceRequestRequest) GetIdempotencyKey() string {
	if x != nil {
		return x.IdempotencyKey
	}
	return ""
}

type CompleteInsuranceRequestResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Request       *Request               `protobuf:"bytes,1,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CompleteInsuranceRequestResponse) Reset() {
	*x = CompleteInsuranceRequestResponse{}
	mi := &file_services_request_request_proto_msgTypes[68]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CompleteInsuranceRequestResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CompleteInsuranceRequestResponse) ProtoMessage() {}

func (x *CompleteInsuranceRequestResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[68]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CompleteInsuranceRequestResponse.ProtoReflect.Descriptor instead.
func (*CompleteInsuranceRequestResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{68}
}

func (x *CompleteInsuranceRequestResponse) GetRequest() *Request {
	if x != nil {
		return x.Request
	}
	return nil
}

type ReserveRequestBidSelectionRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	BidId          int64                  `protobuf:"varint,2,opt,name=bid_id,json=bidId,proto3" json:"bid_id,omitempty"`
	PayerUserId    int64                  `protobuf:"varint,3,opt,name=payer_user_id,json=payerUserId,proto3" json:"payer_user_id,omitempty"`
	OrganizationId string                 `protobuf:"bytes,4,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ReserveRequestBidSelectionRequest) Reset() {
	*x = ReserveRequestBidSelectionRequest{}
	mi := &file_services_request_request_proto_msgTypes[69]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReserveRequestBidSelectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReserveRequestBidSelectionRequest) ProtoMessage() {}

func (x *ReserveRequestBidSelectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[69]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReserveRequestBidSelectionRequest.ProtoReflect.Descriptor instead.
func (*ReserveRequestBidSelectionRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{69}
}

func (x *ReserveRequestBidSelectionRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *ReserveRequestBidSelectionRequest) GetBidId() int64 {
	if x != nil {
		return x.BidId
	}
	return 0
}

func (x *ReserveRequestBidSelectionRequest) GetPayerUserId() int64 {
	if x != nil {
		return x.PayerUserId
	}
	return 0
}

func (x *ReserveRequestBidSelectionRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

type ReserveRequestBidSelectionResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Reserved        bool                   `protobuf:"varint,1,opt,name=reserved,proto3" json:"reserved,omitempty"`
	AlreadyReserved bool                   `protobuf:"varint,2,opt,name=already_reserved,json=alreadyReserved,proto3" json:"already_reserved,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ReserveRequestBidSelectionResponse) Reset() {
	*x = ReserveRequestBidSelectionResponse{}
	mi := &file_services_request_request_proto_msgTypes[70]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReserveRequestBidSelectionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReserveRequestBidSelectionResponse) ProtoMessage() {}

func (x *ReserveRequestBidSelectionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[70]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReserveRequestBidSelectionResponse.ProtoReflect.Descriptor instead.
func (*ReserveRequestBidSelectionResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{70}
}

func (x *ReserveRequestBidSelectionResponse) GetReserved() bool {
	if x != nil {
		return x.Reserved
	}
	return false
}

func (x *ReserveRequestBidSelectionResponse) GetAlreadyReserved() bool {
	if x != nil {
		return x.AlreadyReserved
	}
	return false
}

type CancelRequestBidSelectionRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RequestId      string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	BidId          int64                  `protobuf:"varint,2,opt,name=bid_id,json=bidId,proto3" json:"bid_id,omitempty"`
	PayerUserId    int64                  `protobuf:"varint,3,opt,name=payer_user_id,json=payerUserId,proto3" json:"payer_user_id,omitempty"`
	OrganizationId string                 `protobuf:"bytes,4,opt,name=organization_id,json=organizationId,proto3" json:"organization_id,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *CancelRequestBidSelectionRequest) Reset() {
	*x = CancelRequestBidSelectionRequest{}
	mi := &file_services_request_request_proto_msgTypes[71]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CancelRequestBidSelectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CancelRequestBidSelectionRequest) ProtoMessage() {}

func (x *CancelRequestBidSelectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[71]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CancelRequestBidSelectionRequest.ProtoReflect.Descriptor instead.
func (*CancelRequestBidSelectionRequest) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{71}
}

func (x *CancelRequestBidSelectionRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *CancelRequestBidSelectionRequest) GetBidId() int64 {
	if x != nil {
		return x.BidId
	}
	return 0
}

func (x *CancelRequestBidSelectionRequest) GetPayerUserId() int64 {
	if x != nil {
		return x.PayerUserId
	}
	return 0
}

func (x *CancelRequestBidSelectionRequest) GetOrganizationId() string {
	if x != nil {
		return x.OrganizationId
	}
	return ""
}

type CancelRequestBidSelectionResponse struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	Cancelled        bool                   `protobuf:"varint,1,opt,name=cancelled,proto3" json:"cancelled,omitempty"`
	AlreadyCancelled bool                   `protobuf:"varint,2,opt,name=already_cancelled,json=alreadyCancelled,proto3" json:"already_cancelled,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *CancelRequestBidSelectionResponse) Reset() {
	*x = CancelRequestBidSelectionResponse{}
	mi := &file_services_request_request_proto_msgTypes[72]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CancelRequestBidSelectionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CancelRequestBidSelectionResponse) ProtoMessage() {}

func (x *CancelRequestBidSelectionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_services_request_request_proto_msgTypes[72]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CancelRequestBidSelectionResponse.ProtoReflect.Descriptor instead.
func (*CancelRequestBidSelectionResponse) Descriptor() ([]byte, []int) {
	return file_services_request_request_proto_rawDescGZIP(), []int{72}
}

func (x *CancelRequestBidSelectionResponse) GetCancelled() bool {
	if x != nil {
		return x.Cancelled
	}
	return false
}

func (x *CancelRequestBidSelectionResponse) GetAlreadyCancelled() bool {
	if x != nil {
		return x.AlreadyCancelled
	}
	return false
}

var File_services_request_request_proto protoreflect.FileDescriptor

const file_services_request_request_proto_rawDesc = "" +
	"\n" +
	"\x1eservices/request/request.proto\x12\x13services.request.v1\x1a\x1fgoogle/protobuf/timestamp.proto\"`\n" +
	"\fEstimateItem\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x14\n" +
	"\x05hours\x18\x02 \x01(\x01R\x05hours\x12\x14\n" +
	"\x05price\x18\x03 \x01(\x03R\x05price\x12\x10\n" +
	"\x03qty\x18\x04 \x01(\x05R\x03qty\"\xa6\x0f\n" +
	"\aRequest\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x124\n" +
	"\x04type\x18\x02 \x01(\x0e2 .services.request.v1.RequestTypeR\x04type\x12:\n" +
	"\x06status\x18\x03 \x01(\x0e2\".services.request.v1.RequestStatusR\x06status\x12\x17\n" +
	"\auser_id\x18\x04 \x01(\x03R\x06userId\x12\x19\n" +
	"\bgroup_id\x18\x05 \x01(\x03R\agroupId\x12!\n" +
	"\fcategory_ids\x18\x06 \x03(\x03R\vcategoryIds\x12\x1e\n" +
	"\vcar_make_id\x18\a \x01(\x03R\tcarMakeId\x12 \n" +
	"\fcar_model_id\x18\b \x01(\x03R\n" +
	"carModelId\x12/\n" +
	"\x11car_generation_id\x18\t \x01(\x03H\x00R\x0fcarGenerationId\x88\x01\x01\x12\x12\n" +
	"\x04year\x18\n" +
	" \x01(\x05R\x04year\x12\x12\n" +
	"\x04note\x18\v \x01(\tR\x04note\x12\x16\n" +
	"\x06photos\x18\f \x03(\tR\x06photos\x12\x17\n" +
	"\acity_id\x18\r \x01(\x03R\x06cityId\x12\x18\n" +
	"\aaddress\x18\x0e \x01(\tR\aaddress\x12\x1a\n" +
	"\blatitude\x18\x0f \x01(\x01R\blatitude\x12\x1c\n" +
	"\tlongitude\x18\x10 \x01(\x01R\tlongitude\x129\n" +
	"\n" +
	"created_at\x18\x11 \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x129\n" +
	"\n" +
	"updated_at\x18\x12 \x01(\v2\x1a.google.protobuf.TimestampR\tupdatedAt\x12B\n" +
	"\fpublished_at\x18\x13 \x01(\v2\x1a.google.protobuf.TimestampH\x01R\vpublishedAt\x88\x01\x01\x12+\n" +
	"\x0frepair_order_id\x18\x14 \x01(\x03H\x02R\rrepairOrderId\x88\x01\x01\x12\x1a\n" +
	"\x06org_id\x18\x15 \x01(\tH\x03R\x05orgId\x88\x01\x01\x12\x15\n" +
	"\x06is_new\x18\x16 \x01(\bR\x05isNew\x12\x1d\n" +
	"\n" +
	"bids_count\x18\x17 \x01(\x05R\tbidsCount\x12<\n" +
	"\tpaused_at\x18\x18 \x01(\v2\x1a.google.protobuf.TimestampH\x04R\bpausedAt\x88\x01\x01\x12,\n" +
	"\x10paused_by_org_id\x18\x19 \x01(\tH\x05R\rpausedByOrgId\x88\x01\x01\x12 \n" +
	"\tlegacy_id\x18\x1a \x01(\x03H\x06R\blegacyId\x88\x01\x01\x12'\n" +
	"\rgarage_car_id\x18\x1b \x01(\x03H\aR\vgarageCarId\x88\x01\x01\x124\n" +
	"\x04kind\x18\x1c \x01(\x0e2 .services.request.v1.RequestKindR\x04kind\x12&\n" +
	"\fpayout_price\x18\x1d \x01(\x03H\bR\vpayoutPrice\x88\x01\x01\x12;\n" +
	"\bdeadline\x18\x1e \x01(\v2\x1a.google.protobuf.TimestampH\tR\bdeadline\x88\x01\x01\x12L\n" +
	"\x0fassignment_mode\x18\x1f \x01(\x0e2#.services.request.v1.AssignmentModeR\x0eassignmentMode\x12'\n" +
	"\rwork_order_id\x18  \x01(\x03H\n" +
	"R\vworkOrderId\x88\x01\x01\x12H\n" +
	"\x0eestimate_items\x18! \x03(\v2!.services.request.v1.EstimateItemR\restimateItems\x12.\n" +
	"\x11claimed_by_org_id\x18\" \x01(\tH\vR\x0eclaimedByOrgId\x88\x01\x01\x12\x1b\n" +
	"\tever_held\x18# \x01(\bR\beverHeld\x12+\n" +
	"\x0fselected_bid_id\x18$ \x01(\x03H\fR\rselectedBidId\x88\x01\x01\x12=\n" +
	"\x18selected_organization_id\x18% \x01(\tH\rR\x16selectedOrganizationId\x88\x01\x01\x12J\n" +
	"\fescrow_state\x18& \x01(\x0e2'.services.request.v1.RequestEscrowStateR\vescrowState\x127\n" +
	"\x15escrow_transaction_id\x18' \x01(\x03H\x0eR\x13escrowTransactionId\x88\x01\x01B\x14\n" +
	"\x12_car_generation_idB\x0f\n" +
	"\r_published_atB\x12\n" +
	"\x10_repair_order_idB\t\n" +
	"\a_org_idB\f\n" +
	"\n" +
	"_paused_atB\x13\n" +
	"\x11_paused_by_org_idB\f\n" +
	"\n" +
	"_legacy_idB\x10\n" +
	"\x0e_garage_car_idB\x0f\n" +
	"\r_payout_priceB\v\n" +
	"\t_deadlineB\x10\n" +
	"\x0e_work_order_idB\x14\n" +
	"\x12_claimed_by_org_idB\x12\n" +
	"\x10_selected_bid_idB\x1b\n" +
	"\x19_selected_organization_idB\x18\n" +
	"\x16_escrow_transaction_id\"\xd9\t\n" +
	"\x14CreateRequestRequest\x124\n" +
	"\x04type\x18\x01 \x01(\x0e2 .services.request.v1.RequestTypeR\x04type\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\x03R\x06userId\x12\x19\n" +
	"\bgroup_id\x18\x03 \x01(\x03R\agroupId\x12!\n" +
	"\fcategory_ids\x18\x04 \x03(\x03R\vcategoryIds\x12\x1e\n" +
	"\vcar_make_id\x18\x05 \x01(\x03R\tcarMakeId\x12 \n" +
	"\fcar_model_id\x18\x06 \x01(\x03R\n" +
	"carModelId\x12/\n" +
	"\x11car_generation_id\x18\a \x01(\x03H\x00R\x0fcarGenerationId\x88\x01\x01\x12\x12\n" +
	"\x04year\x18\b \x01(\x05R\x04year\x12\x12\n" +
	"\x04note\x18\t \x01(\tR\x04note\x12\x16\n" +
	"\x06photos\x18\n" +
	" \x03(\tR\x06photos\x12\x17\n" +
	"\acity_id\x18\v \x01(\x03R\x06cityId\x12\x18\n" +
	"\aaddress\x18\f \x01(\tR\aaddress\x12\x1a\n" +
	"\blatitude\x18\r \x01(\x01R\blatitude\x12\x1c\n" +
	"\tlongitude\x18\x0e \x01(\x01R\tlongitude\x12\x18\n" +
	"\apublish\x18\x0f \x01(\bR\apublish\x12+\n" +
	"\x0frepair_order_id\x18\x10 \x01(\x03H\x01R\rrepairOrderId\x88\x01\x01\x12\x1a\n" +
	"\x06org_id\x18\x11 \x01(\tH\x02R\x05orgId\x88\x01\x01\x12'\n" +
	"\rgarage_car_id\x18\x12 \x01(\x03H\x03R\vgarageCarId\x88\x01\x01\x124\n" +
	"\x04kind\x18\x13 \x01(\x0e2 .services.request.v1.RequestKindR\x04kind\x12&\n" +
	"\fpayout_price\x18\x14 \x01(\x03H\x04R\vpayoutPrice\x88\x01\x01\x12;\n" +
	"\bdeadline\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampH\x05R\bdeadline\x88\x01\x01\x12L\n" +
	"\x0fassignment_mode\x18\x16 \x01(\x0e2#.services.request.v1.AssignmentModeR\x0eassignmentMode\x12'\n" +
	"\rwork_order_id\x18\x17 \x01(\x03H\x06R\vworkOrderId\x88\x01\x01\x12H\n" +
	"\x0eestimate_items\x18\x18 \x03(\v2!.services.request.v1.EstimateItemR\restimateItems\x12$\n" +
	"\x0etarget_org_ids\x18\x19 \x03(\tR\ftargetOrgIds\x12S\n" +
	"\x0einsurance_info\x18\x1a \x01(\v2'.services.request.v1.InsuranceInfoInputH\aR\rinsuranceInfo\x88\x01\x01\x12'\n" +
	"\x0fidempotency_key\x18\x1b \x01(\tR\x0eidempotencyKeyB\x14\n" +
	"\x12_car_generation_idB\x12\n" +
	"\x10_repair_order_idB\t\n" +
	"\a_org_idB\x10\n" +
	"\x0e_garage_car_idB\x0f\n" +
	"\r_payout_priceB\v\n" +
	"\t_deadlineB\x10\n" +
	"\x0e_work_order_idB\x11\n" +
	"\x0f_insurance_info\"O\n" +
	"\x15CreateRequestResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"2\n" +
	"\x11GetRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"L\n" +
	"\x12GetRequestResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"\x9d\x05\n" +
	"\x14UpdateRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\x03R\x06userId\x12\x17\n" +
	"\x04note\x18\x03 \x01(\tH\x00R\x04note\x88\x01\x01\x12\x16\n" +
	"\x06photos\x18\x04 \x03(\tR\x06photos\x12\x1d\n" +
	"\aaddress\x18\x05 \x01(\tH\x01R\aaddress\x88\x01\x01\x12\x1f\n" +
	"\blatitude\x18\x06 \x01(\x01H\x02R\blatitude\x88\x01\x01\x12!\n" +
	"\tlongitude\x18\a \x01(\x01H\x03R\tlongitude\x88\x01\x01\x12#\n" +
	"\vcar_make_id\x18\b \x01(\x03H\x04R\tcarMakeId\x88\x01\x01\x12%\n" +
	"\fcar_model_id\x18\t \x01(\x03H\x05R\n" +
	"carModelId\x88\x01\x01\x12/\n" +
	"\x11car_generation_id\x18\n" +
	" \x01(\x03H\x06R\x0fcarGenerationId\x88\x01\x01\x12\x17\n" +
	"\x04year\x18\v \x01(\x05H\aR\x04year\x88\x01\x01\x12'\n" +
	"\rgarage_car_id\x18\f \x01(\x03H\bR\vgarageCarId\x88\x01\x01\x12\x1e\n" +
	"\bgroup_id\x18\r \x01(\x03H\tR\agroupId\x88\x01\x01\x12!\n" +
	"\fcategory_ids\x18\x0e \x03(\x03R\vcategoryIds\x12(\n" +
	"\x10has_category_ids\x18\x0f \x01(\bR\x0ehasCategoryIdsB\a\n" +
	"\x05_noteB\n" +
	"\n" +
	"\b_addressB\v\n" +
	"\t_latitudeB\f\n" +
	"\n" +
	"_longitudeB\x0e\n" +
	"\f_car_make_idB\x0f\n" +
	"\r_car_model_idB\x14\n" +
	"\x12_car_generation_idB\a\n" +
	"\x05_yearB\x10\n" +
	"\x0e_garage_car_idB\v\n" +
	"\t_group_id\"O\n" +
	"\x15UpdateRequestResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"N\n" +
	"\x14DeleteRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\x03R\x06userId\"1\n" +
	"\x15DeleteRequestResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\"\xf9\x04\n" +
	"\x13ListRequestsRequest\x124\n" +
	"\x04type\x18\x01 \x01(\x0e2 .services.request.v1.RequestTypeR\x04type\x12:\n" +
	"\x06status\x18\x02 \x01(\x0e2\".services.request.v1.RequestStatusR\x06status\x12\x17\n" +
	"\auser_id\x18\x03 \x01(\x03R\x06userId\x12\x19\n" +
	"\bgroup_id\x18\x04 \x01(\x03R\agroupId\x12\x1f\n" +
	"\vcategory_id\x18\x05 \x01(\x03R\n" +
	"categoryId\x12\x17\n" +
	"\acity_id\x18\x06 \x01(\x03R\x06cityId\x12\x1e\n" +
	"\vcar_make_id\x18\a \x01(\x03R\tcarMakeId\x12\x12\n" +
	"\x04page\x18\b \x01(\x05R\x04page\x12\x1b\n" +
	"\tpage_size\x18\t \x01(\x05R\bpageSize\x12\x17\n" +
	"\asort_by\x18\n" +
	" \x01(\tR\x06sortBy\x12+\n" +
	"\x0frepair_order_id\x18\v \x01(\x03H\x00R\rrepairOrderId\x88\x01\x01\x12,\n" +
	"\x0forganization_id\x18\f \x01(\tH\x01R\x0eorganizationId\x88\x01\x01\x127\n" +
	"\x15owner_organization_id\x18\r \x01(\tH\x02R\x13ownerOrganizationId\x88\x01\x01\x129\n" +
	"\x04kind\x18\x0e \x01(\x0e2 .services.request.v1.RequestKindH\x03R\x04kind\x88\x01\x01B\x12\n" +
	"\x10_repair_order_idB\x12\n" +
	"\x10_organization_idB\x18\n" +
	"\x16_owner_organization_idB\a\n" +
	"\x05_kind\"f\n" +
	"\x14ListRequestsResponse\x128\n" +
	"\brequests\x18\x01 \x03(\v2\x1c.services.request.v1.RequestR\brequests\x12\x14\n" +
	"\x05total\x18\x02 \x01(\x05R\x05total\"\xa9\x01\n" +
	"\x1cListPublishedPreviewsRequest\x12\x1f\n" +
	"\vcategory_id\x18\x01 \x01(\x03R\n" +
	"categoryId\x12\x17\n" +
	"\acity_id\x18\x02 \x01(\x03R\x06cityId\x12\x1e\n" +
	"\vcar_make_id\x18\x03 \x01(\x03R\tcarMakeId\x12\x12\n" +
	"\x04page\x18\x04 \x01(\x05R\x04page\x12\x1b\n" +
	"\tpage_size\x18\x05 \x01(\x05R\bpageSize\"\xf7\x02\n" +
	"\x17PublishedRequestPreview\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x124\n" +
	"\x04type\x18\x02 \x01(\x0e2 .services.request.v1.RequestTypeR\x04type\x12\x19\n" +
	"\bgroup_id\x18\x03 \x01(\x03R\agroupId\x12!\n" +
	"\fcategory_ids\x18\x04 \x03(\x03R\vcategoryIds\x12\x1e\n" +
	"\vcar_make_id\x18\x05 \x01(\x03R\tcarMakeId\x12 \n" +
	"\fcar_model_id\x18\x06 \x01(\x03R\n" +
	"carModelId\x12\x12\n" +
	"\x04year\x18\a \x01(\x05R\x04year\x12\x12\n" +
	"\x04note\x18\b \x01(\tR\x04note\x12\x16\n" +
	"\x06photos\x18\t \x03(\tR\x06photos\x12\x17\n" +
	"\acity_id\x18\n" +
	" \x01(\x03R\x06cityId\x12=\n" +
	"\fpublished_at\x18\v \x01(\v2\x1a.google.protobuf.TimestampR\vpublishedAt\"\x7f\n" +
	"\x1dListPublishedPreviewsResponse\x12H\n" +
	"\brequests\x18\x01 \x03(\v2,.services.request.v1.PublishedRequestPreviewR\brequests\x12\x14\n" +
	"\x05total\x18\x02 \x01(\x05R\x05total\"\xe3\x02\n" +
	"\x15SearchRequestsRequest\x12\x14\n" +
	"\x05query\x18\x01 \x01(\tR\x05query\x124\n" +
	"\x04type\x18\x02 \x01(\x0e2 .services.request.v1.RequestTypeR\x04type\x12\x19\n" +
	"\bgroup_id\x18\x03 \x01(\x03R\agroupId\x12!\n" +
	"\fcategory_ids\x18\x04 \x03(\x03R\vcategoryIds\x12\x17\n" +
	"\acity_id\x18\x05 \x01(\x03R\x06cityId\x12\x1e\n" +
	"\vcar_make_id\x18\x06 \x01(\x03R\tcarMakeId\x12 \n" +
	"\fcar_model_id\x18\a \x01(\x03R\n" +
	"carModelId\x12\x19\n" +
	"\byear_min\x18\b \x01(\x05R\ayearMin\x12\x19\n" +
	"\byear_max\x18\t \x01(\x05R\ayearMax\x12\x12\n" +
	"\x04page\x18\n" +
	" \x01(\x05R\x04page\x12\x1b\n" +
	"\tpage_size\x18\v \x01(\x05R\bpageSize\"h\n" +
	"\x16SearchRequestsResponse\x128\n" +
	"\brequests\x18\x01 \x03(\v2\x1c.services.request.v1.RequestR\brequests\x12\x14\n" +
	"\x05total\x18\x02 \x01(\x05R\x05total\"\x89\x01\n" +
	"\x13ChangeStatusRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12:\n" +
	"\x06status\x18\x02 \x01(\x0e2\".services.request.v1.RequestStatusR\x06status\x12\x17\n" +
	"\auser_id\x18\x03 \x01(\x03R\x06userId\"N\n" +
	"\x14ChangeStatusResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"\x9b\x02\n" +
	"\x1bPrepareRequestEscrowRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06bid_id\x18\x02 \x01(\x03R\x05bidId\x12\"\n" +
	"\rpayer_user_id\x18\x03 \x01(\x03R\vpayerUserId\x12:\n" +
	"\x19recipient_organization_id\x18\x04 \x01(\tR\x17recipientOrganizationId\x12!\n" +
	"\famount_minor\x18\x05 \x01(\x03R\vamountMinor\x12\x1a\n" +
	"\bcurrency\x18\x06 \x01(\tR\bcurrency\x12'\n" +
	"\x0fidempotency_key\x18\a \x01(\tR\x0eidempotencyKey\"\xcf\x02\n" +
	"\x1cPrepareRequestEscrowResponse\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06bid_id\x18\x02 \x01(\x03R\x05bidId\x12\"\n" +
	"\rpayer_user_id\x18\x03 \x01(\x03R\vpayerUserId\x12:\n" +
	"\x19recipient_organization_id\x18\x04 \x01(\tR\x17recipientOrganizationId\x12!\n" +
	"\famount_minor\x18\x05 \x01(\x03R\vamountMinor\x12\x1a\n" +
	"\bcurrency\x18\x06 \x01(\tR\bcurrency\x12=\n" +
	"\x05state\x18\a \x01(\x0e2'.services.request.v1.RequestEscrowStateR\x05state\x12\x1b\n" +
	"\tever_held\x18\b \x01(\bR\beverHeld\"\xa2\x02\n" +
	"$AuthorizeRequestEscrowCaptureRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06bid_id\x18\x02 \x01(\x03R\x05bidId\x12\"\n" +
	"\rpayer_user_id\x18\x03 \x01(\x03R\vpayerUserId\x12:\n" +
	"\x19recipient_organization_id\x18\x04 \x01(\tR\x17recipientOrganizationId\x12!\n" +
	"\famount_minor\x18\x05 \x01(\x03R\vamountMinor\x12\x1a\n" +
	"\bcurrency\x18\x06 \x01(\tR\bcurrency\x12%\n" +
	"\x0etransaction_id\x18\a \x01(\x03R\rtransactionId\"j\n" +
	"%AuthorizeRequestEscrowCaptureResponse\x12\x1e\n" +
	"\n" +
	"authorized\x18\x01 \x01(\bR\n" +
	"authorized\x12!\n" +
	"\falready_held\x18\x02 \x01(\bR\valreadyHeld\"\xe4\x01\n" +
	"\x1dMarkRequestEscrowStateRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06bid_id\x18\x02 \x01(\x03R\x05bidId\x12%\n" +
	"\x0etransaction_id\x18\x03 \x01(\x03R\rtransactionId\x12=\n" +
	"\x05state\x18\x04 \x01(\x0e2'.services.request.v1.RequestEscrowStateR\x05state\x12'\n" +
	"\x0fidempotency_key\x18\x05 \x01(\tR\x0eidempotencyKey\"\x86\x01\n" +
	"\x1eMarkRequestEscrowStateResponse\x12=\n" +
	"\x05state\x18\x01 \x01(\x0e2'.services.request.v1.RequestEscrowStateR\x05state\x12%\n" +
	"\x0erequest_closed\x18\x02 \x01(\bR\rrequestClosed\"\x9c\x01\n" +
	"\x17SelectRequestBidRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06bid_id\x18\x02 \x01(\x03R\x05bidId\x12\"\n" +
	"\rpayer_user_id\x18\x03 \x01(\x03R\vpayerUserId\x12'\n" +
	"\x0forganization_id\x18\x04 \x01(\tR\x0eorganizationId\"a\n" +
	"\x18SelectRequestBidResponse\x12\x1a\n" +
	"\bselected\x18\x01 \x01(\bR\bselected\x12)\n" +
	"\x10already_selected\x18\x02 \x01(\bR\x0falreadySelected\"v\n" +
	"\x13CloseRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\x03R\x06userId\x12'\n" +
	"\x0fidempotency_key\x18\x03 \x01(\tR\x0eidempotencyKey\"N\n" +
	"\x14CloseRequestResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"\xde\x01\n" +
	"\x16GetUserRequestsRequest\x12\x17\n" +
	"\auser_id\x18\x01 \x01(\x03R\x06userId\x12:\n" +
	"\x06status\x18\x02 \x01(\x0e2\".services.request.v1.RequestStatusR\x06status\x12\x12\n" +
	"\x04page\x18\x03 \x01(\x05R\x04page\x12\x1b\n" +
	"\tpage_size\x18\x04 \x01(\x05R\bpageSize\x12>\n" +
	"\bstatuses\x18\x05 \x03(\x0e2\".services.request.v1.RequestStatusR\bstatuses\"i\n" +
	"\x17GetUserRequestsResponse\x128\n" +
	"\brequests\x18\x01 \x03(\v2\x1c.services.request.v1.RequestR\brequests\x12\x14\n" +
	"\x05total\x18\x02 \x01(\x05R\x05total\"6\n" +
	"\x15IncrementViewsRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"2\n" +
	"\x16IncrementViewsResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\"C\n" +
	"\x15GetSuggestionsRequest\x12\x14\n" +
	"\x05query\x18\x01 \x01(\tR\x05query\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\":\n" +
	"\x16GetSuggestionsResponse\x12 \n" +
	"\vsuggestions\x18\x01 \x03(\tR\vsuggestions\"O\n" +
	"$GetNewRequestsForOrganizationRequest\x12'\n" +
	"\x0forganization_id\x18\x01 \x01(\tR\x0eorganizationId\"H\n" +
	"%GetNewRequestsForOrganizationResponse\x12\x1f\n" +
	"\vrequest_ids\x18\x01 \x03(\tR\n" +
	"requestIds\"d\n" +
	"\x1aMarkRequestAsViewedRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12'\n" +
	"\x0forganization_id\x18\x02 \x01(\tR\x0eorganizationId\"7\n" +
	"\x1bMarkRequestAsViewedResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\"]\n" +
	"\x13IsRequestNewRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12'\n" +
	"\x0forganization_id\x18\x02 \x01(\tR\x0eorganizationId\"-\n" +
	"\x14IsRequestNewResponse\x12\x15\n" +
	"\x06is_new\x18\x01 \x01(\bR\x05isNew\"_\n" +
	"\x15DismissRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12'\n" +
	"\x0forganization_id\x18\x02 \x01(\tR\x0eorganizationId\"2\n" +
	"\x16DismissRequestResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\"\xad\x02\n" +
	"!CountUnreadForOrganizationRequest\x12'\n" +
	"\x0forganization_id\x18\x01 \x01(\tR\x0eorganizationId\x124\n" +
	"\x04type\x18\x02 \x01(\x0e2 .services.request.v1.RequestTypeR\x04type\x12\x19\n" +
	"\bgroup_id\x18\x03 \x01(\x03R\agroupId\x12\x1f\n" +
	"\vcategory_id\x18\x04 \x01(\x03R\n" +
	"categoryId\x12\x17\n" +
	"\acity_id\x18\x05 \x01(\x03R\x06cityId\x12\x1e\n" +
	"\vcar_make_id\x18\x06 \x01(\x03R\tcarMakeId\x12 \n" +
	"\fcar_model_id\x18\a \x01(\x03R\n" +
	"carModelId\x12\x12\n" +
	"\x04year\x18\b \x01(\x05R\x04year\":\n" +
	"\"CountUnreadForOrganizationResponse\x12\x14\n" +
	"\x05count\x18\x01 \x01(\x03R\x05count\"\xb2\x01\n" +
	"\x16ClassifyRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x19\n" +
	"\bgroup_id\x18\x02 \x01(\x03R\agroupId\x12!\n" +
	"\fcategory_ids\x18\x03 \x03(\x03R\vcategoryIds\x12!\n" +
	"\fcleaned_note\x18\x04 \x01(\tR\vcleanedNote\x12\x18\n" +
	"\apublish\x18\x05 \x01(\bR\apublish\"Q\n" +
	"\x17ClassifyRequestResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"C\n" +
	"\"GetRequestForClassificationRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"9\n" +
	"#GetRequestForClassificationResponse\x12\x12\n" +
	"\x04note\x18\x01 \x01(\tR\x04note\"?\n" +
	"\x1eGetInsurancePayoutTermsRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"\xdb\x02\n" +
	"\x1fGetInsurancePayoutTermsResponse\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x124\n" +
	"\x04kind\x18\x02 \x01(\x0e2 .services.request.v1.RequestKindR\x04kind\x12:\n" +
	"\x06status\x18\x03 \x01(\x0e2\".services.request.v1.RequestStatusR\x06status\x12.\n" +
	"\x13payout_amount_minor\x18\x04 \x01(\x03R\x11payoutAmountMinor\x12L\n" +
	"\x0fassignment_mode\x18\x05 \x01(\x0e2#.services.request.v1.AssignmentModeR\x0eassignmentMode\x12)\n" +
	"\x11claimed_by_org_id\x18\x06 \x01(\tR\x0eclaimedByOrgId\"A\n" +
	" GetRequestEligibilityInfoRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"\xb3\x01\n" +
	"!GetRequestEligibilityInfoResponse\x12\x17\n" +
	"\auser_id\x18\x01 \x01(\x03R\x06userId\x124\n" +
	"\x04type\x18\x02 \x01(\x0e2 .services.request.v1.RequestTypeR\x04type\x12+\n" +
	"\x0frepair_order_id\x18\x03 \x01(\x03H\x00R\rrepairOrderId\x88\x01\x01B\x12\n" +
	"\x10_repair_order_id\"l\n" +
	"\x1bGetUserRequestCountsRequest\x12\x17\n" +
	"\auser_id\x18\x01 \x01(\x03R\x06userId\x124\n" +
	"\x04type\x18\x02 \x01(\x0e2 .services.request.v1.RequestTypeR\x04type\"\xa0\x01\n" +
	"\x1cGetUserRequestCountsResponse\x12\x14\n" +
	"\x05total\x18\x01 \x01(\x05R\x05total\x12\x16\n" +
	"\x06active\x18\x02 \x01(\x05R\x06active\x12\x1e\n" +
	"\n" +
	"moderation\x18\x03 \x01(\x05R\n" +
	"moderation\x12\x1a\n" +
	"\binactive\x18\x04 \x01(\x05R\binactive\x12\x16\n" +
	"\x06paused\x18\x05 \x01(\x05R\x06paused\"\x90\x01\n" +
	"\x13PauseRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\x03R\x06userId\x12,\n" +
	"\x10paused_by_org_id\x18\x03 \x01(\tH\x00R\rpausedByOrgId\x88\x01\x01B\x13\n" +
	"\x11_paused_by_org_id\"N\n" +
	"\x14PauseRequestResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"O\n" +
	"\x15UnpauseRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\x03R\x06userId\"P\n" +
	"\x16UnpauseRequestResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"\xf7\x01\n" +
	"\x12InsuranceInfoInput\x12'\n" +
	"\x0finsurance_price\x18\x01 \x01(\x03R\x0einsurancePrice\x12+\n" +
	"\x11insurance_company\x18\x02 \x01(\tR\x10insuranceCompany\x12\x1f\n" +
	"\vclient_name\x18\x03 \x01(\tR\n" +
	"clientName\x12!\n" +
	"\fclient_phone\x18\x04 \x01(\tR\vclientPhone\x12\x1d\n" +
	"\n" +
	"min_rating\x18\x05 \x01(\x01R\tminRating\x12(\n" +
	"\x10min_raters_count\x18\x06 \x01(\x05R\x0eminRatersCount\"\x84\x03\n" +
	"\x14RequestInsuranceInfo\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12'\n" +
	"\x0finsurance_price\x18\x02 \x01(\x03R\x0einsurancePrice\x12+\n" +
	"\x11insurance_company\x18\x03 \x01(\tR\x10insuranceCompany\x12\x1f\n" +
	"\vclient_name\x18\x04 \x01(\tR\n" +
	"clientName\x12!\n" +
	"\fclient_phone\x18\x05 \x01(\tR\vclientPhone\x12\x1d\n" +
	"\n" +
	"min_rating\x18\x06 \x01(\x01R\tminRating\x12(\n" +
	"\x10min_raters_count\x18\a \x01(\x05R\x0eminRatersCount\x12$\n" +
	"\x0etarget_org_ids\x18\b \x03(\tR\ftargetOrgIds\x12.\n" +
	"\x11claimed_by_org_id\x18\t \x01(\tH\x00R\x0eclaimedByOrgId\x88\x01\x01B\x14\n" +
	"\x12_claimed_by_org_id\"?\n" +
	"\x1eGetRequestInsuranceInfoRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"`\n" +
	"\x1fGetRequestInsuranceInfoResponse\x12=\n" +
	"\x04info\x18\x01 \x01(\v2).services.request.v1.RequestInsuranceInfoR\x04info\"T\n" +
	"\x1cClaimInsuranceRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06org_id\x18\x02 \x01(\tR\x05orgId\"d\n" +
	"\x1dClaimInsuranceRequestResponse\x12\x18\n" +
	"\aclaimed\x18\x01 \x01(\bR\aclaimed\x12)\n" +
	"\x11claimed_by_org_id\x18\x02 \x01(\tR\x0eclaimedByOrgId\"L\n" +
	"\x14IsOrgTargetedRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06org_id\x18\x02 \x01(\tR\x05orgId\"3\n" +
	"\x15IsOrgTargetedResponse\x12\x1a\n" +
	"\btargeted\x18\x01 \x01(\bR\btargeted\"\x92\x01\n" +
	"\x1fCompleteInsuranceRequestRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12'\n" +
	"\x0forganization_id\x18\x02 \x01(\tR\x0eorganizationId\x12'\n" +
	"\x0fidempotency_key\x18\x03 \x01(\tR\x0eidempotencyKey\"Z\n" +
	" CompleteInsuranceRequestResponse\x126\n" +
	"\arequest\x18\x01 \x01(\v2\x1c.services.request.v1.RequestR\arequest\"\xa6\x01\n" +
	"!ReserveRequestBidSelectionRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06bid_id\x18\x02 \x01(\x03R\x05bidId\x12\"\n" +
	"\rpayer_user_id\x18\x03 \x01(\x03R\vpayerUserId\x12'\n" +
	"\x0forganization_id\x18\x04 \x01(\tR\x0eorganizationId\"k\n" +
	"\"ReserveRequestBidSelectionResponse\x12\x1a\n" +
	"\breserved\x18\x01 \x01(\bR\breserved\x12)\n" +
	"\x10already_reserved\x18\x02 \x01(\bR\x0falreadyReserved\"\xa5\x01\n" +
	" CancelRequestBidSelectionRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x15\n" +
	"\x06bid_id\x18\x02 \x01(\x03R\x05bidId\x12\"\n" +
	"\rpayer_user_id\x18\x03 \x01(\x03R\vpayerUserId\x12'\n" +
	"\x0forganization_id\x18\x04 \x01(\tR\x0eorganizationId\"n\n" +
	"!CancelRequestBidSelectionResponse\x12\x1c\n" +
	"\tcancelled\x18\x01 \x01(\bR\tcancelled\x12+\n" +
	"\x11already_cancelled\x18\x02 \x01(\bR\x10alreadyCancelled*\\\n" +
	"\vRequestType\x12\x1c\n" +
	"\x18REQUEST_TYPE_UNSPECIFIED\x10\x00\x12\x17\n" +
	"\x13REQUEST_TYPE_REPAIR\x10\x01\x12\x16\n" +
	"\x12REQUEST_TYPE_PARTS\x10\x02*\xbe\x01\n" +
	"\rRequestStatus\x12\x1e\n" +
	"\x1aREQUEST_STATUS_UNSPECIFIED\x10\x00\x12\x1d\n" +
	"\x19REQUEST_STATUS_MODERATION\x10\x01\x12\x1c\n" +
	"\x18REQUEST_STATUS_PUBLISHED\x10\x02\x12\x1a\n" +
	"\x16REQUEST_STATUS_DELETED\x10\x03\x12\x19\n" +
	"\x15REQUEST_STATUS_CLOSED\x10\x04\x12\x19\n" +
	"\x15REQUEST_STATUS_PAUSED\x10\x05*\x8e\x02\n" +
	"\x12RequestEscrowState\x12$\n" +
	" REQUEST_ESCROW_STATE_UNSPECIFIED\x10\x00\x12\"\n" +
	"\x1eREQUEST_ESCROW_STATE_INITIATED\x10\x01\x12\x1d\n" +
	"\x19REQUEST_ESCROW_STATE_HELD\x10\x02\x12&\n" +
	"\"REQUEST_ESCROW_STATE_WORK_DECLARED\x10\x03\x12!\n" +
	"\x1dREQUEST_ESCROW_STATE_DISPUTED\x10\x04\x12!\n" +
	"\x1dREQUEST_ESCROW_STATE_RELEASED\x10\x05\x12!\n" +
	"\x1dREQUEST_ESCROW_STATE_RETURNED\x10\x06*a\n" +
	"\vRequestKind\x12\x1c\n" +
	"\x18REQUEST_KIND_UNSPECIFIED\x10\x00\x12\x18\n" +
	"\x14REQUEST_KIND_REGULAR\x10\x01\x12\x1a\n" +
	"\x16REQUEST_KIND_INSURANCE\x10\x02*m\n" +
	"\x0eAssignmentMode\x12\x1f\n" +
	"\x1bASSIGNMENT_MODE_UNSPECIFIED\x10\x00\x12\x1a\n" +
	"\x16ASSIGNMENT_MODE_MANUAL\x10\x01\x12\x1e\n" +
	"\x1aASSIGNMENT_MODE_AUTO_FIRST\x10\x022\xfb\x1f\n" +
	"\x0eRequestService\x12f\n" +
	"\rCreateRequest\x12).services.request.v1.CreateRequestRequest\x1a*.services.request.v1.CreateRequestResponse\x12]\n" +
	"\n" +
	"GetRequest\x12&.services.request.v1.GetRequestRequest\x1a'.services.request.v1.GetRequestResponse\x12f\n" +
	"\rUpdateRequest\x12).services.request.v1.UpdateRequestRequest\x1a*.services.request.v1.UpdateRequestResponse\x12f\n" +
	"\rDeleteRequest\x12).services.request.v1.DeleteRequestRequest\x1a*.services.request.v1.DeleteRequestResponse\x12c\n" +
	"\fListRequests\x12(.services.request.v1.ListRequestsRequest\x1a).services.request.v1.ListRequestsResponse\x12~\n" +
	"\x15ListPublishedPreviews\x121.services.request.v1.ListPublishedPreviewsRequest\x1a2.services.request.v1.ListPublishedPreviewsResponse\x12i\n" +
	"\x0eSearchRequests\x12*.services.request.v1.SearchRequestsRequest\x1a+.services.request.v1.SearchRequestsResponse\x12c\n" +
	"\fChangeStatus\x12(.services.request.v1.ChangeStatusRequest\x1a).services.request.v1.ChangeStatusResponse\x12l\n" +
	"\x0fGetUserRequests\x12+.services.request.v1.GetUserRequestsRequest\x1a,.services.request.v1.GetUserRequestsResponse\x12i\n" +
	"\x0eIncrementViews\x12*.services.request.v1.IncrementViewsRequest\x1a+.services.request.v1.IncrementViewsResponse\x12i\n" +
	"\x0eGetSuggestions\x12*.services.request.v1.GetSuggestionsRequest\x1a+.services.request.v1.GetSuggestionsResponse\x12\x96\x01\n" +
	"\x1dGetNewRequestsForOrganization\x129.services.request.v1.GetNewRequestsForOrganizationRequest\x1a:.services.request.v1.GetNewRequestsForOrganizationResponse\x12x\n" +
	"\x13MarkRequestAsViewed\x12/.services.request.v1.MarkRequestAsViewedRequest\x1a0.services.request.v1.MarkRequestAsViewedResponse\x12c\n" +
	"\fIsRequestNew\x12(.services.request.v1.IsRequestNewRequest\x1a).services.request.v1.IsRequestNewResponse\x12i\n" +
	"\x0eDismissRequest\x12*.services.request.v1.DismissRequestRequest\x1a+.services.request.v1.DismissRequestResponse\x12\x8d\x01\n" +
	"\x1aCountUnreadForOrganization\x126.services.request.v1.CountUnreadForOrganizationRequest\x1a7.services.request.v1.CountUnreadForOrganizationResponse\x12l\n" +
	"\x0fClassifyRequest\x12+.services.request.v1.ClassifyRequestRequest\x1a,.services.request.v1.ClassifyRequestResponse\x12\x90\x01\n" +
	"\x1bGetRequestForClassification\x127.services.request.v1.GetRequestForClassificationRequest\x1a8.services.request.v1.GetRequestForClassificationResponse\x12\x84\x01\n" +
	"\x17GetInsurancePayoutTerms\x123.services.request.v1.GetInsurancePayoutTermsRequest\x1a4.services.request.v1.GetInsurancePayoutTermsResponse\x12\x8a\x01\n" +
	"\x19GetRequestEligibilityInfo\x125.services.request.v1.GetRequestEligibilityInfoRequest\x1a6.services.request.v1.GetRequestEligibilityInfoResponse\x12{\n" +
	"\x14GetUserRequestCounts\x120.services.request.v1.GetUserRequestCountsRequest\x1a1.services.request.v1.GetUserRequestCountsResponse\x12c\n" +
	"\fPauseRequest\x12(.services.request.v1.PauseRequestRequest\x1a).services.request.v1.PauseRequestResponse\x12i\n" +
	"\x0eUnpauseRequest\x12*.services.request.v1.UnpauseRequestRequest\x1a+.services.request.v1.UnpauseRequestResponse\x12\x84\x01\n" +
	"\x17GetRequestInsuranceInfo\x123.services.request.v1.GetRequestInsuranceInfoRequest\x1a4.services.request.v1.GetRequestInsuranceInfoResponse\x12~\n" +
	"\x15ClaimInsuranceRequest\x121.services.request.v1.ClaimInsuranceRequestRequest\x1a2.services.request.v1.ClaimInsuranceRequestResponse\x12\x87\x01\n" +
	"\x18CompleteInsuranceRequest\x124.services.request.v1.CompleteInsuranceRequestRequest\x1a5.services.request.v1.CompleteInsuranceRequestResponse\x12f\n" +
	"\rIsOrgTargeted\x12).services.request.v1.IsOrgTargetedRequest\x1a*.services.request.v1.IsOrgTargetedResponse\x12{\n" +
	"\x14PrepareRequestEscrow\x120.services.request.v1.PrepareRequestEscrowRequest\x1a1.services.request.v1.PrepareRequestEscrowResponse\x12\x96\x01\n" +
	"\x1dAuthorizeRequestEscrowCapture\x129.services.request.v1.AuthorizeRequestEscrowCaptureRequest\x1a:.services.request.v1.AuthorizeRequestEscrowCaptureResponse\x12\x81\x01\n" +
	"\x16MarkRequestEscrowState\x122.services.request.v1.MarkRequestEscrowStateRequest\x1a3.services.request.v1.MarkRequestEscrowStateResponse\x12o\n" +
	"\x10SelectRequestBid\x12,.services.request.v1.SelectRequestBidRequest\x1a-.services.request.v1.SelectRequestBidResponse\x12c\n" +
	"\fCloseRequest\x12(.services.request.v1.CloseRequestRequest\x1a).services.request.v1.CloseRequestResponse\x12\x8d\x01\n" +
	"\x1aReserveRequestBidSelection\x126.services.request.v1.ReserveRequestBidSelectionRequest\x1a7.services.request.v1.ReserveRequestBidSelectionResponse\x12\x8a\x01\n" +
	"\x19CancelRequestBidSelection\x125.services.request.v1.CancelRequestBidSelectionRequest\x1a6.services.request.v1.CancelRequestBidSelectionResponseB=Z;github.com/4ubak/cg-proto/gen/go/services/request;requestv1b\x06proto3"

var (
	file_services_request_request_proto_rawDescOnce sync.Once
	file_services_request_request_proto_rawDescData []byte
)

func file_services_request_request_proto_rawDescGZIP() []byte {
	file_services_request_request_proto_rawDescOnce.Do(func() {
		file_services_request_request_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_services_request_request_proto_rawDesc), len(file_services_request_request_proto_rawDesc)))
	})
	return file_services_request_request_proto_rawDescData
}

var file_services_request_request_proto_enumTypes = make([]protoimpl.EnumInfo, 5)
var file_services_request_request_proto_msgTypes = make([]protoimpl.MessageInfo, 73)
var file_services_request_request_proto_goTypes = []any{
	(RequestType)(0),                              // 0: services.request.v1.RequestType
	(RequestStatus)(0),                            // 1: services.request.v1.RequestStatus
	(RequestEscrowState)(0),                       // 2: services.request.v1.RequestEscrowState
	(RequestKind)(0),                              // 3: services.request.v1.RequestKind
	(AssignmentMode)(0),                           // 4: services.request.v1.AssignmentMode
	(*EstimateItem)(nil),                          // 5: services.request.v1.EstimateItem
	(*Request)(nil),                               // 6: services.request.v1.Request
	(*CreateRequestRequest)(nil),                  // 7: services.request.v1.CreateRequestRequest
	(*CreateRequestResponse)(nil),                 // 8: services.request.v1.CreateRequestResponse
	(*GetRequestRequest)(nil),                     // 9: services.request.v1.GetRequestRequest
	(*GetRequestResponse)(nil),                    // 10: services.request.v1.GetRequestResponse
	(*UpdateRequestRequest)(nil),                  // 11: services.request.v1.UpdateRequestRequest
	(*UpdateRequestResponse)(nil),                 // 12: services.request.v1.UpdateRequestResponse
	(*DeleteRequestRequest)(nil),                  // 13: services.request.v1.DeleteRequestRequest
	(*DeleteRequestResponse)(nil),                 // 14: services.request.v1.DeleteRequestResponse
	(*ListRequestsRequest)(nil),                   // 15: services.request.v1.ListRequestsRequest
	(*ListRequestsResponse)(nil),                  // 16: services.request.v1.ListRequestsResponse
	(*ListPublishedPreviewsRequest)(nil),          // 17: services.request.v1.ListPublishedPreviewsRequest
	(*PublishedRequestPreview)(nil),               // 18: services.request.v1.PublishedRequestPreview
	(*ListPublishedPreviewsResponse)(nil),         // 19: services.request.v1.ListPublishedPreviewsResponse
	(*SearchRequestsRequest)(nil),                 // 20: services.request.v1.SearchRequestsRequest
	(*SearchRequestsResponse)(nil),                // 21: services.request.v1.SearchRequestsResponse
	(*ChangeStatusRequest)(nil),                   // 22: services.request.v1.ChangeStatusRequest
	(*ChangeStatusResponse)(nil),                  // 23: services.request.v1.ChangeStatusResponse
	(*PrepareRequestEscrowRequest)(nil),           // 24: services.request.v1.PrepareRequestEscrowRequest
	(*PrepareRequestEscrowResponse)(nil),          // 25: services.request.v1.PrepareRequestEscrowResponse
	(*AuthorizeRequestEscrowCaptureRequest)(nil),  // 26: services.request.v1.AuthorizeRequestEscrowCaptureRequest
	(*AuthorizeRequestEscrowCaptureResponse)(nil), // 27: services.request.v1.AuthorizeRequestEscrowCaptureResponse
	(*MarkRequestEscrowStateRequest)(nil),         // 28: services.request.v1.MarkRequestEscrowStateRequest
	(*MarkRequestEscrowStateResponse)(nil),        // 29: services.request.v1.MarkRequestEscrowStateResponse
	(*SelectRequestBidRequest)(nil),               // 30: services.request.v1.SelectRequestBidRequest
	(*SelectRequestBidResponse)(nil),              // 31: services.request.v1.SelectRequestBidResponse
	(*CloseRequestRequest)(nil),                   // 32: services.request.v1.CloseRequestRequest
	(*CloseRequestResponse)(nil),                  // 33: services.request.v1.CloseRequestResponse
	(*GetUserRequestsRequest)(nil),                // 34: services.request.v1.GetUserRequestsRequest
	(*GetUserRequestsResponse)(nil),               // 35: services.request.v1.GetUserRequestsResponse
	(*IncrementViewsRequest)(nil),                 // 36: services.request.v1.IncrementViewsRequest
	(*IncrementViewsResponse)(nil),                // 37: services.request.v1.IncrementViewsResponse
	(*GetSuggestionsRequest)(nil),                 // 38: services.request.v1.GetSuggestionsRequest
	(*GetSuggestionsResponse)(nil),                // 39: services.request.v1.GetSuggestionsResponse
	(*GetNewRequestsForOrganizationRequest)(nil),  // 40: services.request.v1.GetNewRequestsForOrganizationRequest
	(*GetNewRequestsForOrganizationResponse)(nil), // 41: services.request.v1.GetNewRequestsForOrganizationResponse
	(*MarkRequestAsViewedRequest)(nil),            // 42: services.request.v1.MarkRequestAsViewedRequest
	(*MarkRequestAsViewedResponse)(nil),           // 43: services.request.v1.MarkRequestAsViewedResponse
	(*IsRequestNewRequest)(nil),                   // 44: services.request.v1.IsRequestNewRequest
	(*IsRequestNewResponse)(nil),                  // 45: services.request.v1.IsRequestNewResponse
	(*DismissRequestRequest)(nil),                 // 46: services.request.v1.DismissRequestRequest
	(*DismissRequestResponse)(nil),                // 47: services.request.v1.DismissRequestResponse
	(*CountUnreadForOrganizationRequest)(nil),     // 48: services.request.v1.CountUnreadForOrganizationRequest
	(*CountUnreadForOrganizationResponse)(nil),    // 49: services.request.v1.CountUnreadForOrganizationResponse
	(*ClassifyRequestRequest)(nil),                // 50: services.request.v1.ClassifyRequestRequest
	(*ClassifyRequestResponse)(nil),               // 51: services.request.v1.ClassifyRequestResponse
	(*GetRequestForClassificationRequest)(nil),    // 52: services.request.v1.GetRequestForClassificationRequest
	(*GetRequestForClassificationResponse)(nil),   // 53: services.request.v1.GetRequestForClassificationResponse
	(*GetInsurancePayoutTermsRequest)(nil),        // 54: services.request.v1.GetInsurancePayoutTermsRequest
	(*GetInsurancePayoutTermsResponse)(nil),       // 55: services.request.v1.GetInsurancePayoutTermsResponse
	(*GetRequestEligibilityInfoRequest)(nil),      // 56: services.request.v1.GetRequestEligibilityInfoRequest
	(*GetRequestEligibilityInfoResponse)(nil),     // 57: services.request.v1.GetRequestEligibilityInfoResponse
	(*GetUserRequestCountsRequest)(nil),           // 58: services.request.v1.GetUserRequestCountsRequest
	(*GetUserRequestCountsResponse)(nil),          // 59: services.request.v1.GetUserRequestCountsResponse
	(*PauseRequestRequest)(nil),                   // 60: services.request.v1.PauseRequestRequest
	(*PauseRequestResponse)(nil),                  // 61: services.request.v1.PauseRequestResponse
	(*UnpauseRequestRequest)(nil),                 // 62: services.request.v1.UnpauseRequestRequest
	(*UnpauseRequestResponse)(nil),                // 63: services.request.v1.UnpauseRequestResponse
	(*InsuranceInfoInput)(nil),                    // 64: services.request.v1.InsuranceInfoInput
	(*RequestInsuranceInfo)(nil),                  // 65: services.request.v1.RequestInsuranceInfo
	(*GetRequestInsuranceInfoRequest)(nil),        // 66: services.request.v1.GetRequestInsuranceInfoRequest
	(*GetRequestInsuranceInfoResponse)(nil),       // 67: services.request.v1.GetRequestInsuranceInfoResponse
	(*ClaimInsuranceRequestRequest)(nil),          // 68: services.request.v1.ClaimInsuranceRequestRequest
	(*ClaimInsuranceRequestResponse)(nil),         // 69: services.request.v1.ClaimInsuranceRequestResponse
	(*IsOrgTargetedRequest)(nil),                  // 70: services.request.v1.IsOrgTargetedRequest
	(*IsOrgTargetedResponse)(nil),                 // 71: services.request.v1.IsOrgTargetedResponse
	(*CompleteInsuranceRequestRequest)(nil),       // 72: services.request.v1.CompleteInsuranceRequestRequest
	(*CompleteInsuranceRequestResponse)(nil),      // 73: services.request.v1.CompleteInsuranceRequestResponse
	(*ReserveRequestBidSelectionRequest)(nil),     // 74: services.request.v1.ReserveRequestBidSelectionRequest
	(*ReserveRequestBidSelectionResponse)(nil),    // 75: services.request.v1.ReserveRequestBidSelectionResponse
	(*CancelRequestBidSelectionRequest)(nil),      // 76: services.request.v1.CancelRequestBidSelectionRequest
	(*CancelRequestBidSelectionResponse)(nil),     // 77: services.request.v1.CancelRequestBidSelectionResponse
	(*timestamppb.Timestamp)(nil),                 // 78: google.protobuf.Timestamp
}
var file_services_request_request_proto_depIdxs = []int32{
	0,  // 0: services.request.v1.Request.type:type_name -> services.request.v1.RequestType
	1,  // 1: services.request.v1.Request.status:type_name -> services.request.v1.RequestStatus
	78, // 2: services.request.v1.Request.created_at:type_name -> google.protobuf.Timestamp
	78, // 3: services.request.v1.Request.updated_at:type_name -> google.protobuf.Timestamp
	78, // 4: services.request.v1.Request.published_at:type_name -> google.protobuf.Timestamp
	78, // 5: services.request.v1.Request.paused_at:type_name -> google.protobuf.Timestamp
	3,  // 6: services.request.v1.Request.kind:type_name -> services.request.v1.RequestKind
	78, // 7: services.request.v1.Request.deadline:type_name -> google.protobuf.Timestamp
	4,  // 8: services.request.v1.Request.assignment_mode:type_name -> services.request.v1.AssignmentMode
	5,  // 9: services.request.v1.Request.estimate_items:type_name -> services.request.v1.EstimateItem
	2,  // 10: services.request.v1.Request.escrow_state:type_name -> services.request.v1.RequestEscrowState
	0,  // 11: services.request.v1.CreateRequestRequest.type:type_name -> services.request.v1.RequestType
	3,  // 12: services.request.v1.CreateRequestRequest.kind:type_name -> services.request.v1.RequestKind
	78, // 13: services.request.v1.CreateRequestRequest.deadline:type_name -> google.protobuf.Timestamp
	4,  // 14: services.request.v1.CreateRequestRequest.assignment_mode:type_name -> services.request.v1.AssignmentMode
	5,  // 15: services.request.v1.CreateRequestRequest.estimate_items:type_name -> services.request.v1.EstimateItem
	64, // 16: services.request.v1.CreateRequestRequest.insurance_info:type_name -> services.request.v1.InsuranceInfoInput
	6,  // 17: services.request.v1.CreateRequestResponse.request:type_name -> services.request.v1.Request
	6,  // 18: services.request.v1.GetRequestResponse.request:type_name -> services.request.v1.Request
	6,  // 19: services.request.v1.UpdateRequestResponse.request:type_name -> services.request.v1.Request
	0,  // 20: services.request.v1.ListRequestsRequest.type:type_name -> services.request.v1.RequestType
	1,  // 21: services.request.v1.ListRequestsRequest.status:type_name -> services.request.v1.RequestStatus
	3,  // 22: services.request.v1.ListRequestsRequest.kind:type_name -> services.request.v1.RequestKind
	6,  // 23: services.request.v1.ListRequestsResponse.requests:type_name -> services.request.v1.Request
	0,  // 24: services.request.v1.PublishedRequestPreview.type:type_name -> services.request.v1.RequestType
	78, // 25: services.request.v1.PublishedRequestPreview.published_at:type_name -> google.protobuf.Timestamp
	18, // 26: services.request.v1.ListPublishedPreviewsResponse.requests:type_name -> services.request.v1.PublishedRequestPreview
	0,  // 27: services.request.v1.SearchRequestsRequest.type:type_name -> services.request.v1.RequestType
	6,  // 28: services.request.v1.SearchRequestsResponse.requests:type_name -> services.request.v1.Request
	1,  // 29: services.request.v1.ChangeStatusRequest.status:type_name -> services.request.v1.RequestStatus
	6,  // 30: services.request.v1.ChangeStatusResponse.request:type_name -> services.request.v1.Request
	2,  // 31: services.request.v1.PrepareRequestEscrowResponse.state:type_name -> services.request.v1.RequestEscrowState
	2,  // 32: services.request.v1.MarkRequestEscrowStateRequest.state:type_name -> services.request.v1.RequestEscrowState
	2,  // 33: services.request.v1.MarkRequestEscrowStateResponse.state:type_name -> services.request.v1.RequestEscrowState
	6,  // 34: services.request.v1.CloseRequestResponse.request:type_name -> services.request.v1.Request
	1,  // 35: services.request.v1.GetUserRequestsRequest.status:type_name -> services.request.v1.RequestStatus
	1,  // 36: services.request.v1.GetUserRequestsRequest.statuses:type_name -> services.request.v1.RequestStatus
	6,  // 37: services.request.v1.GetUserRequestsResponse.requests:type_name -> services.request.v1.Request
	0,  // 38: services.request.v1.CountUnreadForOrganizationRequest.type:type_name -> services.request.v1.RequestType
	6,  // 39: services.request.v1.ClassifyRequestResponse.request:type_name -> services.request.v1.Request
	3,  // 40: services.request.v1.GetInsurancePayoutTermsResponse.kind:type_name -> services.request.v1.RequestKind
	1,  // 41: services.request.v1.GetInsurancePayoutTermsResponse.status:type_name -> services.request.v1.RequestStatus
	4,  // 42: services.request.v1.GetInsurancePayoutTermsResponse.assignment_mode:type_name -> services.request.v1.AssignmentMode
	0,  // 43: services.request.v1.GetRequestEligibilityInfoResponse.type:type_name -> services.request.v1.RequestType
	0,  // 44: services.request.v1.GetUserRequestCountsRequest.type:type_name -> services.request.v1.RequestType
	6,  // 45: services.request.v1.PauseRequestResponse.request:type_name -> services.request.v1.Request
	6,  // 46: services.request.v1.UnpauseRequestResponse.request:type_name -> services.request.v1.Request
	65, // 47: services.request.v1.GetRequestInsuranceInfoResponse.info:type_name -> services.request.v1.RequestInsuranceInfo
	6,  // 48: services.request.v1.CompleteInsuranceRequestResponse.request:type_name -> services.request.v1.Request
	7,  // 49: services.request.v1.RequestService.CreateRequest:input_type -> services.request.v1.CreateRequestRequest
	9,  // 50: services.request.v1.RequestService.GetRequest:input_type -> services.request.v1.GetRequestRequest
	11, // 51: services.request.v1.RequestService.UpdateRequest:input_type -> services.request.v1.UpdateRequestRequest
	13, // 52: services.request.v1.RequestService.DeleteRequest:input_type -> services.request.v1.DeleteRequestRequest
	15, // 53: services.request.v1.RequestService.ListRequests:input_type -> services.request.v1.ListRequestsRequest
	17, // 54: services.request.v1.RequestService.ListPublishedPreviews:input_type -> services.request.v1.ListPublishedPreviewsRequest
	20, // 55: services.request.v1.RequestService.SearchRequests:input_type -> services.request.v1.SearchRequestsRequest
	22, // 56: services.request.v1.RequestService.ChangeStatus:input_type -> services.request.v1.ChangeStatusRequest
	34, // 57: services.request.v1.RequestService.GetUserRequests:input_type -> services.request.v1.GetUserRequestsRequest
	36, // 58: services.request.v1.RequestService.IncrementViews:input_type -> services.request.v1.IncrementViewsRequest
	38, // 59: services.request.v1.RequestService.GetSuggestions:input_type -> services.request.v1.GetSuggestionsRequest
	40, // 60: services.request.v1.RequestService.GetNewRequestsForOrganization:input_type -> services.request.v1.GetNewRequestsForOrganizationRequest
	42, // 61: services.request.v1.RequestService.MarkRequestAsViewed:input_type -> services.request.v1.MarkRequestAsViewedRequest
	44, // 62: services.request.v1.RequestService.IsRequestNew:input_type -> services.request.v1.IsRequestNewRequest
	46, // 63: services.request.v1.RequestService.DismissRequest:input_type -> services.request.v1.DismissRequestRequest
	48, // 64: services.request.v1.RequestService.CountUnreadForOrganization:input_type -> services.request.v1.CountUnreadForOrganizationRequest
	50, // 65: services.request.v1.RequestService.ClassifyRequest:input_type -> services.request.v1.ClassifyRequestRequest
	52, // 66: services.request.v1.RequestService.GetRequestForClassification:input_type -> services.request.v1.GetRequestForClassificationRequest
	54, // 67: services.request.v1.RequestService.GetInsurancePayoutTerms:input_type -> services.request.v1.GetInsurancePayoutTermsRequest
	56, // 68: services.request.v1.RequestService.GetRequestEligibilityInfo:input_type -> services.request.v1.GetRequestEligibilityInfoRequest
	58, // 69: services.request.v1.RequestService.GetUserRequestCounts:input_type -> services.request.v1.GetUserRequestCountsRequest
	60, // 70: services.request.v1.RequestService.PauseRequest:input_type -> services.request.v1.PauseRequestRequest
	62, // 71: services.request.v1.RequestService.UnpauseRequest:input_type -> services.request.v1.UnpauseRequestRequest
	66, // 72: services.request.v1.RequestService.GetRequestInsuranceInfo:input_type -> services.request.v1.GetRequestInsuranceInfoRequest
	68, // 73: services.request.v1.RequestService.ClaimInsuranceRequest:input_type -> services.request.v1.ClaimInsuranceRequestRequest
	72, // 74: services.request.v1.RequestService.CompleteInsuranceRequest:input_type -> services.request.v1.CompleteInsuranceRequestRequest
	70, // 75: services.request.v1.RequestService.IsOrgTargeted:input_type -> services.request.v1.IsOrgTargetedRequest
	24, // 76: services.request.v1.RequestService.PrepareRequestEscrow:input_type -> services.request.v1.PrepareRequestEscrowRequest
	26, // 77: services.request.v1.RequestService.AuthorizeRequestEscrowCapture:input_type -> services.request.v1.AuthorizeRequestEscrowCaptureRequest
	28, // 78: services.request.v1.RequestService.MarkRequestEscrowState:input_type -> services.request.v1.MarkRequestEscrowStateRequest
	30, // 79: services.request.v1.RequestService.SelectRequestBid:input_type -> services.request.v1.SelectRequestBidRequest
	32, // 80: services.request.v1.RequestService.CloseRequest:input_type -> services.request.v1.CloseRequestRequest
	74, // 81: services.request.v1.RequestService.ReserveRequestBidSelection:input_type -> services.request.v1.ReserveRequestBidSelectionRequest
	76, // 82: services.request.v1.RequestService.CancelRequestBidSelection:input_type -> services.request.v1.CancelRequestBidSelectionRequest
	8,  // 83: services.request.v1.RequestService.CreateRequest:output_type -> services.request.v1.CreateRequestResponse
	10, // 84: services.request.v1.RequestService.GetRequest:output_type -> services.request.v1.GetRequestResponse
	12, // 85: services.request.v1.RequestService.UpdateRequest:output_type -> services.request.v1.UpdateRequestResponse
	14, // 86: services.request.v1.RequestService.DeleteRequest:output_type -> services.request.v1.DeleteRequestResponse
	16, // 87: services.request.v1.RequestService.ListRequests:output_type -> services.request.v1.ListRequestsResponse
	19, // 88: services.request.v1.RequestService.ListPublishedPreviews:output_type -> services.request.v1.ListPublishedPreviewsResponse
	21, // 89: services.request.v1.RequestService.SearchRequests:output_type -> services.request.v1.SearchRequestsResponse
	23, // 90: services.request.v1.RequestService.ChangeStatus:output_type -> services.request.v1.ChangeStatusResponse
	35, // 91: services.request.v1.RequestService.GetUserRequests:output_type -> services.request.v1.GetUserRequestsResponse
	37, // 92: services.request.v1.RequestService.IncrementViews:output_type -> services.request.v1.IncrementViewsResponse
	39, // 93: services.request.v1.RequestService.GetSuggestions:output_type -> services.request.v1.GetSuggestionsResponse
	41, // 94: services.request.v1.RequestService.GetNewRequestsForOrganization:output_type -> services.request.v1.GetNewRequestsForOrganizationResponse
	43, // 95: services.request.v1.RequestService.MarkRequestAsViewed:output_type -> services.request.v1.MarkRequestAsViewedResponse
	45, // 96: services.request.v1.RequestService.IsRequestNew:output_type -> services.request.v1.IsRequestNewResponse
	47, // 97: services.request.v1.RequestService.DismissRequest:output_type -> services.request.v1.DismissRequestResponse
	49, // 98: services.request.v1.RequestService.CountUnreadForOrganization:output_type -> services.request.v1.CountUnreadForOrganizationResponse
	51, // 99: services.request.v1.RequestService.ClassifyRequest:output_type -> services.request.v1.ClassifyRequestResponse
	53, // 100: services.request.v1.RequestService.GetRequestForClassification:output_type -> services.request.v1.GetRequestForClassificationResponse
	55, // 101: services.request.v1.RequestService.GetInsurancePayoutTerms:output_type -> services.request.v1.GetInsurancePayoutTermsResponse
	57, // 102: services.request.v1.RequestService.GetRequestEligibilityInfo:output_type -> services.request.v1.GetRequestEligibilityInfoResponse
	59, // 103: services.request.v1.RequestService.GetUserRequestCounts:output_type -> services.request.v1.GetUserRequestCountsResponse
	61, // 104: services.request.v1.RequestService.PauseRequest:output_type -> services.request.v1.PauseRequestResponse
	63, // 105: services.request.v1.RequestService.UnpauseRequest:output_type -> services.request.v1.UnpauseRequestResponse
	67, // 106: services.request.v1.RequestService.GetRequestInsuranceInfo:output_type -> services.request.v1.GetRequestInsuranceInfoResponse
	69, // 107: services.request.v1.RequestService.ClaimInsuranceRequest:output_type -> services.request.v1.ClaimInsuranceRequestResponse
	73, // 108: services.request.v1.RequestService.CompleteInsuranceRequest:output_type -> services.request.v1.CompleteInsuranceRequestResponse
	71, // 109: services.request.v1.RequestService.IsOrgTargeted:output_type -> services.request.v1.IsOrgTargetedResponse
	25, // 110: services.request.v1.RequestService.PrepareRequestEscrow:output_type -> services.request.v1.PrepareRequestEscrowResponse
	27, // 111: services.request.v1.RequestService.AuthorizeRequestEscrowCapture:output_type -> services.request.v1.AuthorizeRequestEscrowCaptureResponse
	29, // 112: services.request.v1.RequestService.MarkRequestEscrowState:output_type -> services.request.v1.MarkRequestEscrowStateResponse
	31, // 113: services.request.v1.RequestService.SelectRequestBid:output_type -> services.request.v1.SelectRequestBidResponse
	33, // 114: services.request.v1.RequestService.CloseRequest:output_type -> services.request.v1.CloseRequestResponse
	75, // 115: services.request.v1.RequestService.ReserveRequestBidSelection:output_type -> services.request.v1.ReserveRequestBidSelectionResponse
	77, // 116: services.request.v1.RequestService.CancelRequestBidSelection:output_type -> services.request.v1.CancelRequestBidSelectionResponse
	83, // [83:117] is the sub-list for method output_type
	49, // [49:83] is the sub-list for method input_type
	49, // [49:49] is the sub-list for extension type_name
	49, // [49:49] is the sub-list for extension extendee
	0,  // [0:49] is the sub-list for field type_name
}

func init() { file_services_request_request_proto_init() }
func file_services_request_request_proto_init() {
	if File_services_request_request_proto != nil {
		return
	}
	file_services_request_request_proto_msgTypes[1].OneofWrappers = []any{}
	file_services_request_request_proto_msgTypes[2].OneofWrappers = []any{}
	file_services_request_request_proto_msgTypes[6].OneofWrappers = []any{}
	file_services_request_request_proto_msgTypes[10].OneofWrappers = []any{}
	file_services_request_request_proto_msgTypes[52].OneofWrappers = []any{}
	file_services_request_request_proto_msgTypes[55].OneofWrappers = []any{}
	file_services_request_request_proto_msgTypes[60].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_services_request_request_proto_rawDesc), len(file_services_request_request_proto_rawDesc)),
			NumEnums:      5,
			NumMessages:   73,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_services_request_request_proto_goTypes,
		DependencyIndexes: file_services_request_request_proto_depIdxs,
		EnumInfos:         file_services_request_request_proto_enumTypes,
		MessageInfos:      file_services_request_request_proto_msgTypes,
	}.Build()
	File_services_request_request_proto = out.File
	file_services_request_request_proto_goTypes = nil
	file_services_request_request_proto_depIdxs = nil
}
